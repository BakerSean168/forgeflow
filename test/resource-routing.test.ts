...
test('the default affinity policy is versioned, ranked, native-profile-safe, and duplicate-proof', () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'AFFINITY_POLICY_IMPLEMENTATION_DUPLICATE_RANK',
  );
});
...
test('resource records validate derived tiers and provider-native bindings without secrets', () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'RESOURCE_TIER_DERIVATION_INVALID',
  );
});
...
test('selector hard-gates executable profiles on runtime admission readiness', () => {
  ...
  const readiness = {
    isReady(
      candidate: import('../src/core/orchestration/resourceSelector.js').ResourceSelectionCandidate,
    ) {
...
test('fault classification and pure resource state policy follow the ForgeFlow lifecycle rules', () => {
  assert.equal(
...
function databaseFile(prefix: string): string {
  return path.join(fs.mkdtempSync(path.join(os.tmpdir(), prefix)), 'forgeflow.sqlite');
}
...
test('selection and state repositories enforce immutability, CAS, event safety, and restart durability', () => {
  const file = databaseFile('forgeflow-routing-repository-');
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'EXECUTION_RESOURCE_SELECTION_IMMUTABLE',
  );
...
test('v5 databases migrate routing additively through the current schema and malformed routing tables fail closed', () => {
  const file = databaseFile('forgeflow-routing-migration-');
  let db = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
  const executionId = seedExecution(db);
  const planCount = db.prepare('SELECT count(*) AS count FROM plans').get()?.count;
  db.exec(
    "DROP TABLE execution_resource_selections; DROP TABLE resource_state_overrides; UPDATE schema_meta SET schema_version=5 WHERE schema_id='forgeflow';",
  );
  db.close();
  db = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
  assert.equal(
  ...
  const malformed = databaseFile('forgeflow-routing-malformed-');
  db = openDatabase(malformed, { environment: 'test', env: { NODE_ENV: 'test' } });
  db.exec(
    "DROP TABLE execution_resource_selections; CREATE TABLE execution_resource_selections (execution_id TEXT PRIMARY KEY); UPDATE schema_meta SET schema_version=5 WHERE schema_id='forgeflow';",
  );
  db.close();
  assert.throws(
    () => openDatabase(malformed, { environment: 'test', env: { NODE_ENV: 'test' } }),
  ...
  assert.equal(
    raw.prepare("SELECT schema_version FROM schema_meta WHERE schema_id='forgeflow'").get()
      ?.schema_version,
    5,
  );
  raw.close();
