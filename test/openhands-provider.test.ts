...
function launchInput(
  ...
  return {
    ...
    workspace: {
      ...
      sourceRepositoryPath: '/repos/forgeflow',
      sourceRevision: 'source-sha',
...
test('OpenHands implementation and review launches use distinct models, tools and safe provenance', async () => {
  ...
  assert.match(
    ...
    /\/workspace\/executions\/exec-1\/repo\/\.forgeflow-completion-evidence\.json/,
  );
  assert.match(
    implementationBody.initial_message.content[0].text,
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'OPENHANDS_IMPLEMENTATION_PHASE_REQUIRED',
  );
  await assert.rejects(
    () => review.launch(launchInput('IMPLEMENT')),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'OPENHANDS_REVIEW_PHASE_REQUIRED',
  );
});

test('Managed Codex implementation launches ACP with Responses transport and controller-owned ForgeFlow evidence', async () => {
  const requests: Array<{ url: string; init: RequestInit }> = [];
  const fake = (async (url: string | URL | Request, init: RequestInit = {}) => {
    requests.push({ url: String(url), init });
    const body = JSON.parse(String(init.body)) as Record<string, any>;
  ...
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_DRIVER.value, 'codex');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_ROLE.value, 'worker');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_TRANSPORT.value, 'litellm-managed');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_REASONING_EFFORT.value, 'xhigh');
  assert.equal(body.secrets.FORGEFLOW_LITELLM_BASE_URL.value, 'http://litellm.test/v1');
  assert.equal(body.secrets.FORGEFLOW_LITELLM_API_KEY.value, secrets.liteLlmApiKey);
  assert.equal(body.secrets.FORGEFLOW_EXECUTION_ID.value, 'exec-1');
  assert.equal(
    body.secrets.FORGEFLOW_IMPLEMENTATION_EVIDENCE_PATH.value,
    '/workspace/executions/exec-1/evidence.json',
  );
  assert.equal(body.secrets.FORGEFLOW_EXECUTION_ID.value, 'exec-1');
  assert.equal(body.secrets.FORGEFLOW_SOURCE_SHA.value, 'source-sha');
  assert.equal(body.secrets.FORGEFLOW_IMPLEMENTATION_PHASE.value, 'IMPLEMENT');
...
test('Business Codex implementation injects exact literal-worktree Git provenance', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-provider-worktree-'));
  ...
  input.workspace = {
    ...input.workspace,
    hostPath: worktree,
    executionPath: '/workspace/forgeflow/plans/project/plan/items/work/repo',
    evidenceHostPath: path.join(root, 'evidence.json'),
    evidenceExecutionPath:
      '/workspace/forgeflow/plans/project/plan/items/work/.executions/exec-1/evidence.json',
    sourceRepositoryPath: repository,
    sourceRevision,
  };
  await provider.launch(input);
  const body = JSON.parse(String(requests[0]!.init.body)) as Record<string, any>;
  assert.equal(body.secrets.FORGEFLOW_EXPECTED_GIT_COMMON_DIR.value, path.join(repository, '.git'));
  assert.equal(
    body.secrets.FORGEFLOW_EXPECTED_WORKTREE_GIT_FILE.value,
...
test('Business Codex review launches provider-native ACP with persisted ChatGPT auth and ForgeFlow evidence metadata', async () => {
  const requests: Array<{ url: string; init: RequestInit }> = [];
  const fake = (async (url: string | URL | Request, init: RequestInit = {}) => {
    requests.push({ url: String(url), init });
    const body = JSON.parse(String(init.body)) as Record<string, any>;
  ...
  assert.equal(body.agent.llm, undefined);
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_TRANSPORT.value, 'provider-native');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_ROLE.value, 'review');
  assert.equal(body.secrets.FORGEFLOW_CODEX_AUTH_HOME.value, '/openhands-state/codex-business');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_REASONING_EFFORT.value, 'medium');
  assert.equal(
    body.secrets.FORGEFLOW_REVIEW_EVIDENCE_PATH.value,
    '/workspace/executions/exec-1/evidence.json',
  );
  assert.equal(body.secrets.FORGEFLOW_EXECUTION_ID.value, 'exec-1');
  assert.equal(body.secrets.FORGEFLOW_REVIEWED_SHA.value, 'source-sha');
...
test('model-native factory resolves selected backend, model and transport without model-name routing', async () => {
  ...
  const cases = [
    {
      ...
      expectedSecrets: {
        FORGEFLOW_HEADLESS_DRIVER: 'codex',
        FORGEFLOW_HEADLESS_TRANSPORT: 'litellm-managed',
        FORGEFLOW_LITELLM_API_KEY: secrets.liteLlmApiKey,
        FORGEFLOW_AGENT_MODEL: 'gpt-5.6-luna',
      },
    },
    {
      selection: {
      ...
      expectedSecrets: {
        FORGEFLOW_HEADLESS_ROLE: 'review',
        FORGEFLOW_HEADLESS_MODEL: 'gpt-5.6-sol',
      },
    },
    {
      selection: {
      ...
      expectedSecrets: {
        FORGEFLOW_HEADLESS_TRANSPORT: 'provider-native',
        FORGEFLOW_CODEX_AUTH_HOME: '/openhands-state/codex-business',
      },
    },
    {
      selection: {
      ...
      expectedCommand: ['/opt/forgeflow-tools/harness_agent_launcher.sh', 'dsh-acp'],
      expectedSecrets: {
        DSH_ACP_MODEL: 'deepseek-v4-flash',
        DEEPSEEK_API_KEY: secrets.liteLlmApiKey,
        DEEPSEEK_BASE_URL: 'http://litellm.test/v1',
        FORGEFLOW_EXECUTION_ID: 'exec-1',
        FORGEFLOW_WORKSPACE_REF: '/workspace/executions/exec-1/repo',
    ...
    },
    {
      ...
      expectedCommand: ['/opt/forgeflow-tools/harness_agent_launcher.sh', 'zcode-acp'],
      expectedSecrets: {
    ...
    },
    {
      ...
      expectedSecrets: {
        FORGEFLOW_HEADLESS_DRIVER: 'claude',
        FORGEFLOW_HEADLESS_MODEL: 'claude-opus-5',
        FORGEFLOW_LITELLM_API_KEY: secrets.liteLlmApiKey,
      },
    },
  ] as const;
  ...
  for (const item of cases) {
    ...
    if (item.selection.backend === 'dsh-acp' || item.selection.backend === 'zcode-acp') {
      assert.match(prompt, /\.forgeflow-completion-evidence\.json/);
...
test('model-native ACP runtime probe performs a real bounded turn with probe-only secrets', async () => {
  ...
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_ROLE.value, 'planner');
  assert.equal(body.secrets.FORGEFLOW_HEADLESS_REASONING_EFFORT.value, 'low');
...
test('provider-native Codex does not require LiteLLM credentials and builtin fallback is explicit', async () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'DSH_REVIEW_UNSUPPORTED',
  );
});
...
test('OpenHands recovery is paginated, provenance checked and duplicate-safe', async () => {
  ...
  const fake = (async (url: string | URL | Request) => {
    ...
    const matching = {
      ...
      tags: { execution: 'exec1', project: 'forgeflow', phase: 'implement' },
    };
    if (mode === 'duplicate')
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'OPENHANDS_EXECUTION_DUPLICATE',
  );
  mode = 'mismatch';
  await assert.rejects(
    () => provider.recover(input),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'OPENHANDS_WORKSPACE_PROVENANCE_MISMATCH',
  );
});
...
test('OpenHands status and transport failures fail closed', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'OPENHANDS_HTTP_503',
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'OPENHANDS_TIMEOUT',
  );
});
