...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-delivery-'));
...
test('GitHub delivery runs the repository commitlint contract before pushing and converts failure to typed repair', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'DELIVERY_REQUIRED_CHECK_FAILED',
  );
...
test('GitHub delivery publishes exact-head ForgeFlow governance before satisfying a protected governance context', async () => {
  const value = fixture();
  value.delivery.requiredChecks = ['check', 'ForgeFlow / PR Governance'];
  const calls: Array<{ command: string; args: string[] }> = [];
  let viewed = false;
  const basePr = {
    number: 10,
        ...
        return result(
          JSON.stringify({
            ...
            statusCheckRollup: [
              ...
              { context: 'ForgeFlow / PR Governance', state: 'SUCCESS' },
            ],
  ...
  assert.ok(publish);
  assert.ok(publish!.args.includes('context=ForgeFlow / PR Governance'));
...
test('GitHub delivery discovers an exact already-merged PR for a recovery Plan whose canonical checkout advanced', async () => {
  const value = fixture();
  value.delivery.status = 'PENDING';
  value.delivery.pullRequestNumber = undefined;
  value.delivery.pullRequestUrl = undefined;
  const calls: Array<{ command: string; args: string[] }> = [];
  const mergedPr = {
    number: 165,
    state: 'MERGED',
  ...
  const spawn = ((command: string, args: readonly string[]) => {
    ...
    if (command === '/usr/bin/git') {
      if (argv.includes('check-ref-format')) return result('');
      if (argv.includes('status')) return result('');
      if (argv.includes('rev-parse')) return result('later-canonical\n');
      if (argv.includes('remote') && argv.includes('get-url'))
        return result('https://github.com/acme/repo.git\n');
      if (argv.includes('ls-remote')) return result('later-main\trefs/heads/main\n');
      if (argv.includes('merge-base')) return result('');
      return result('', 'unexpected local command: ' + argv.join(' '), 1);
    }
    if (command === '/usr/bin/gh' && argv[0] === 'pr' && argv[1] === 'list')
  ...
  assert.ok(calls.some((call) => call.command === '/usr/bin/gh' && call.args[1] === 'list'));
  assert.ok(!calls.some((call) => call.command === '/usr/bin/git' && call.args.includes('push')));
...
test('GitHub delivery refuses a durable merged PR whose head is not the exact reviewed Plan SHA', async () => {
  ...
  await assert.rejects(
    () => adapter.advance(value.plan, value.delivery),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'DELIVERY_PR_HEAD_STALE',
  );
  assert.ok(
...
test('GitHub delivery uses an exact linked worktree when canonical HEAD intentionally stays on the base revision', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-delivery-worktree-'));
