...
function downgradeExecutionsToV2(file: string): void {
  ...
  raw.exec(`
    ...
    UPDATE schema_meta SET schema_version=2 WHERE schema_id='forgeflow';
...
function downgradeDeliveryToV4(file: string): void {
  ...
  raw.exec(`
    ...
    UPDATE schema_meta SET schema_version=4 WHERE schema_id='forgeflow';
    PRAGMA legacy_alter_table = OFF;
    PRAGMA foreign_keys = ON;
  `);
  raw.close();
...
function downgradeFixtureToV1(file: string): void {
  ...
  raw.exec(`
    ...
    UPDATE schema_meta SET schema_version=1 WHERE schema_id='forgeflow';
    PRAGMA foreign_keys = ON;
  `);
  raw.close();
}
...
test('runtime schema and checked-in schema.sql remain semantically synchronized', () => {
  const runtime = new DatabaseSync(':memory:');
  runtime.exec(SCHEMA_SQL);
  const fileSchema = new DatabaseSync(':memory:');
  const sqlFile = fs.readFileSync(
    new URL('../src/core/persistence/schema.sql', import.meta.url),
    'utf8',
  );
  fileSchema.exec(sqlFile);
  assert.deepEqual(schemaDescriptor(fileSchema), schemaDescriptor(runtime));
  assert.equal(
    fileSchema.prepare("SELECT schema_version FROM schema_meta WHERE schema_id='forgeflow'").get()
      ?.schema_version,
    SCHEMA_VERSION,
  );
  assert.equal(
    runtime.prepare("SELECT schema_version FROM schema_meta WHERE schema_id='forgeflow'").get()
      ?.schema_version,
    SCHEMA_VERSION,
  );
  runtime.close();
...
test('adapters require the centralized current ForgeFlow schema and never create private tables', () => {
  const raw = new DatabaseSync(':memory:');
  assert.throws(
    () => new JulesAdapter(undefined, raw),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SCHEMA_VERSION_INVALID',
  );
  assert.throws(
...
test('schema v1 migrates in place through v5 and preserves durable production state across restart', () => {
  const file = tempDatabase('forgeflow-migrate-');
  let db = openDatabase(file, { environment: 'production', env: { NODE_ENV: 'production' } });
  ...
  db = openDatabase(file, { environment: 'production', env: { NODE_ENV: 'production' } });
  ...
  assert.ok(
    db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='jules_sessions'")
      .get(),
  );
  assert.ok(
    db
...
test('schema v2 migrates execution phase lineage through v5 without losing queued work', () => {
  const file = tempDatabase('forgeflow-v2-to-v3-');
  let db = openDatabase(file, { environment: 'production', env: { NODE_ENV: 'production' } });
  ...
  const partialFile = tempDatabase('forgeflow-v2-partial-v3-');
  db = openDatabase(partialFile, { environment: 'test', env: { NODE_ENV: 'test' } });
  ...
  raw.close();
  assert.throws(
    () => openDatabase(partialFile, { environment: 'test', env: { NODE_ENV: 'test' } }),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SCHEMA_INCOMPLETE',
  );
  const untouched = new DatabaseSync(partialFile, { readOnly: true });
  assert.equal(
    untouched.prepare("SELECT schema_version FROM schema_meta WHERE schema_id='forgeflow'").get()
      ?.schema_version,
    2,
  );
...
test('schema v4 migrates delivery supersession metadata to v5 without losing existing PR state', () => {
  const file = tempDatabase('forgeflow-to-v5-delivery-');
  let db = openDatabase(file, { environment: 'production', env: { NODE_ENV: 'production' } });
...
test('unknown newer and incomplete ForgeFlow schemas fail closed without resetting durable rows', () => {
  const newerFile = tempDatabase('forgeflow-newer-schema-');
  let db = openDatabase(newerFile, {
  ...
  raw.prepare("UPDATE schema_meta SET schema_version=99 WHERE schema_id='forgeflow'").run();
  raw.close();
  assert.throws(
    () => openDatabase(newerFile, { environment: 'production', env: { NODE_ENV: 'production' } }),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SCHEMA_VERSION_INVALID',
  );
  ...
  const incompleteFile = tempDatabase('forgeflow-incomplete-schema-');
  db = openDatabase(incompleteFile, {
    environment: 'production',
    env: { NODE_ENV: 'production' },
  });
  ...
  const missingColumnFile = tempDatabase('forgeflow-missing-column-');
  db = openDatabase(missingColumnFile, {
    environment: 'production',
    env: { NODE_ENV: 'production' },
  });
  ...
  assert.throws(
    () =>
      openDatabase(missingColumnFile, {
        environment: 'production',
        env: { NODE_ENV: 'production' },
      }),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SCHEMA_INCOMPLETE',
  ...
  const malformedFile = tempDatabase('forgeflow-malformed-schema-');
  db = openDatabase(malformedFile, { environment: 'test', env: { NODE_ENV: 'test' } });
  seed(db, 'malformed-schema-plan');
  db.close();
  raw = new DatabaseSync(malformedFile);
  raw.prepare("DELETE FROM schema_meta WHERE schema_id='forgeflow'").run();
  raw.close();
  assert.throws(
    () =>
      openDatabase(malformedFile, {
        environment: 'test',
        allowDataReset: true,
        env: { NODE_ENV: 'test', FORGEFLOW_ALLOW_DATA_RESET: 'true' },
      }),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SCHEMA_META_MISSING',
  );
...
test('execution sessions and evidence are durable, idempotent, correlated, and immutable', () => {
  ...
  const workspace: WorkspaceDescriptor = {
    ...
    hostPath: '/host/forgeflow/executions/execution-implementation/repo',
    executionPath: '/workspace/forgeflow/executions/execution-implementation/repo',
    evidenceHostPath: '/host/forgeflow/executions/execution-implementation/evidence.json',
    evidenceExecutionPath: '/workspace/forgeflow/executions/execution-implementation/evidence.json',
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'SESSION_WORKSPACE_REVISION_MISMATCH',
  );
  assert.throws(
    () =>
      seeded.repositories.sessions.create({
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SESSION_WORKSPACE_TIME_INVALID',
  );
  assert.throws(
    () =>
      seeded.repositories.sessions.create({
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'SESSION_REPOSITORY_MISMATCH',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'EXECUTION_SESSION_IMMUTABLE',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'PROVIDER_SESSION_IMMUTABLE',
  );
  assert.equal(
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'PROVIDER_OBSERVATION_TIME_INVALID',
  );
  assert.equal(
    seeded.repositories.sessions.updateProviderStatus(
      execution.identity.executionId,
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'DURABLE_EVIDENCE_IMMUTABLE',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'EXECUTION_SESSION_TERMINAL',
  );
  assert.throws(
    () =>
      seeded.repositories.sessions.complete(execution.identity.executionId, {
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'EXECUTION_SESSION_RESULT_IMMUTABLE',
  );
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'CORRUPTED_EXECUTION_SESSION_IDENTITY',
  );
  db.prepare(
  ...
  assert.throws(
    () => seeded.repositories.sessions.get(execution.identity.executionId),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'CORRUPTED_EXECUTION_SESSION_WORKSPACE',
  );
  db.close();
});
...
test('execution leases allow takeover only after expiry', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'DUPLICATE_KEY',
  );
  assert.throws(
    () =>
      seeded.repositories.executions.create({
...
test('plan revision reconciliation and exact work-item acceptance are CAS guarded and audited', () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORK_ITEM_ACCEPTED_REVISION_IMMUTABLE',
  );
...
test('review binding requires a distinct reviewer execution for the same plan and work item', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'REVIEWER_NOT_INDEPENDENT',
  );
  assert.throws(
    () =>
      seeded.repositories.executions.create({
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'EXECUTION_PARENT_REVISION_MISMATCH',
  );
  assert.throws(
    () =>
      seeded.repositories.reviews.attachReviewerExecution(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'REVIEWER_SESSION_INVALID',
  );
  seeded.repositories.sessions.create({
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'REVIEWER_EXECUTION_NOT_SUCCEEDED',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'REVIEWER_EXECUTION_IMMUTABLE',
  );
  assert.equal(
...
test('Jules correlation preserves immutable session lineage and validates remote refresh provenance', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'JULES_SESSION_IMMUTABLE',
  );
  assert.throws(
    () => adapter.getResult('unknown-session'),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'JULES_SESSION_NOT_FOUND',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'JULES_RESULT_IMMUTABLE',
  );
  remote = {
    sessionId: 'jules-session-1',
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'JULES_PROVENANCE_MISMATCH',
  );
  db.close();
});
...
test('maintenance candidate decoding fails closed on corrupted durable evidence', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'CORRUPTED_CANDIDATE_EVIDENCE',
  );
  db.close();
});
...
test('supervisor conversation replacement uses expected-old CAS and emits durable audit evidence', () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'SUPERVISOR_CONVERSATION_IMMUTABLE',
  );
