...
const script = path.join(root, 'scripts/prune-host-cache.sh');
const GiB = 1024 ** 3;
...
function fixture(freeBytes: number, activeExecutions: number, reclaimToBytes?: number) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-host-cache-'));
  ...
  writeExecutable(
    path.join(bin, 'df'),
    `#!/usr/bin/env bash\nset -euo pipefail\nfree=$(cat "$FORGEFLOW_TEST_FREE_FILE")\nkb=$((free / 1024))\nprintf 'Filesystem 1024-blocks Used Available Capacity Mounted on\\n'\nprintf '/dev/fake 200000000 0 %s 0%% /\\n' "$kb"\n`,
  );
  writeExecutable(
    path.join(bin, 'curl'),
    `#!/usr/bin/env bash\nset -euo pipefail\nprintf '{"count":%s,"items":[]}' "$FORGEFLOW_TEST_ACTIVE_EXECUTIONS"\n`,
  );
  writeExecutable(
    path.join(bin, 'docker'),
    `#!/usr/bin/env bash\nset -euo pipefail\nprintf '%s\\n' "$*" >> "$FORGEFLOW_TEST_DOCKER_LOG"\nif [[ -n "${reclaimToBytes ?? ''}" && "$1" == builder && "$2" == prune ]]; then\n  printf '%s\\n' '${reclaimToBytes ?? ''}' > "$FORGEFLOW_TEST_FREE_FILE"\nfi\n`,
  );
  const env = {
    ...process.env,
    PATH: bin + ':' + process.env.PATH,
    FORGEFLOW_HOST_CACHE_TEST_MODE: 'true',
    FORGEFLOW_HOST_CACHE_STATE_FILE: stateFile,
    FORGEFLOW_HOST_CACHE_LOCK: path.join(dir, 'maintenance.lock'),
    FORGEFLOW_RELEASE_LOCK: path.join(dir, 'release.lock'),
    FORGEFLOW_TEST_FREE_FILE: freeFile,
    FORGEFLOW_TEST_DOCKER_LOG: dockerLog,
    FORGEFLOW_TEST_ACTIVE_EXECUTIONS: String(activeExecutions),
  };
...
test('host cache maintenance protects a running ForgeFlow execution', () => {
...
test('host cache maintenance skips while the release lock is held', async () => {
  ...
  const holder = spawn('flock', [value.env.FORGEFLOW_RELEASE_LOCK!, 'sleep', '3'], {
    env: value.env,
    stdio: 'ignore',
  });
  try {
