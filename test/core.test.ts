...
import { transitionPlan } from '../src/core/domain/plan.js';
import { transitionExecution } from '../src/core/domain/execution.js';
import { transitionReview } from '../src/core/domain/review.js';
import { transitionSupervisor } from '../src/core/domain/supervisor.js';
import { transitionAction } from '../src/core/domain/action.js';
import { openDatabase, pragmaValue } from '../src/core/persistence/database.js';
import { resetDatabase } from '../src/core/persistence/reset-database.js';
import { EventStore } from '../src/core/persistence/eventStore.js';
...
function memory() {
  return openDatabase(':memory:', { environment: 'test', env: { NODE_ENV: 'test' } });
}
...
test('fresh database bootstraps once, restarts, enables WAL and foreign keys', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-schema-'));
  const file = path.join(directory, 'control-plane.sqlite');
  const first = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
  ...
  first.close();
  const second = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
...
test('old database fails closed and explicit reset reports scope', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-reset-'));
  const file = path.join(directory, 'legacy.sqlite');
  const legacy = new DatabaseSync(file);
  legacy.exec('CREATE TABLE legacy_state(value TEXT)');
  legacy.close();
  assert.throws(() => openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } }), (error: unknown) => error instanceof ForgeFlowError && error.code === 'DATA_RESET_REQUIRED');
  const result = resetDatabase(file, { environment: 'test', env: { NODE_ENV: 'test', FORGEFLOW_ALLOW_DATA_RESET: 'true' } });
  assert.deepEqual(result, { databaseFile: file, scope: 'ALL_DATA', authorized: true });
  const db = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
  assert.equal(db.prepare("SELECT count(*) AS count FROM schema_meta WHERE schema_id='forgeflow'").get()?.count, 1);
  db.close();
});

test('production reset remains refused even with reset flag', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-prod-reset-'));
  const file = path.join(directory, 'legacy.sqlite');
  const db = new DatabaseSync(file);
  db.exec('CREATE TABLE legacy_state(value TEXT)');
  db.close();
  assert.throws(() => resetDatabase(file, { environment: 'production', env: { NODE_ENV: 'production', FORGEFLOW_ALLOW_DATA_RESET: 'true' } }), (error: unknown) => error instanceof ForgeFlowError && error.code === 'PRODUCTION_RESET_FORBIDDEN');
});
...
test('event append is monotonic, immutable by id and replayable', () => {
  ...
  assert.throws(() => store.append({ eventId: 'event-secret', aggregateId: 'plan-1', aggregateType: 'PLAN', type: 'BAD', payload: { secret: 'x' }, occurredAt: new Date().toISOString(), correlationId: 'corr-1' }), (error: unknown) => error instanceof ForgeFlowError && error.code === 'UNSAFE_EVENT_PAYLOAD');
  db.prepare("UPDATE events SET payload='not-json' WHERE event_id='event-2'").run();
  assert.throws(() => store.listByAggregate('plan-1'), (error: unknown) => error instanceof ForgeFlowError && error.code === 'CORRUPTED_EVENT_PAYLOAD');
  db.close();
});
...
test('domain terminal transitions are monotonic and invalid transitions fail closed', () => {
  ...
  assert.throws(() => transitionPlan({ ...plan, status: 'SUCCEEDED' }, 'RUNNING', at), ForgeFlowError);
  ...
  assert.throws(() => transitionExecution({ ...execution, status: 'SUCCEEDED' }, 'FAILED', at), ForgeFlowError);
  ...
  assert.throws(() => transitionReview(review, 'STALE', at), ForgeFlowError);
  ...
  assert.throws(() => transitionSupervisor(supervisor, 'ACTIVE', at), ForgeFlowError);
  ...
  assert.throws(() => transitionAction(action, 'FAILED', at), ForgeFlowError);
});
...
test('repositories enforce idempotency, CAS, exact review lineage and leases', () => {
  ...
  assert.ok(review);
  assert.throws(() => repos.reviews.create({ idempotencyKey: 'review-2', planId: plan.planId, workItemId: item.workItemId, implementationExecutionId: execution.identity.executionId, sourceRevision: 'sha-c' }), (error: unknown) => error instanceof ForgeFlowError && error.code === 'REVIEW_SOURCE_NOT_IMPLEMENTATION_RESULT');
...
test('parent-child relationships reject duplicates, self cycles and indirect cycles', () => {
  ...
  assert.throws(() => repos.relationships.createParentChild({ parentPlanId: parent.planId, childPlanId: child.planId, kind: 'SYSTEM_REPAIR' }), ForgeFlowError);
  assert.throws(() => repos.relationships.createParentChild({ parentPlanId: child.planId, childPlanId: child.planId, kind: 'FOLLOW_UP' }), ForgeFlowError);
  repos.relationships.createParentChild({ parentPlanId: child.planId, childPlanId: other.planId, kind: 'FOLLOW_UP' });
  assert.throws(() => repos.relationships.createParentChild({ parentPlanId: other.planId, childPlanId: parent.planId, kind: 'FOLLOW_UP' }), ForgeFlowError);
  db.close();
});
...
test('supervisor protocol, bounded projection, stale decision and duplicate wake are safe', async () => {
  ...
  assert.throws(() => parseSupervisorDecision('{"version":1,"version":1}'), (error: unknown) => error instanceof ForgeFlowError && error.code === 'DECISION_DUPLICATE_KEY');
  db.close();
});
...
test('terminal execution result atomically schedules a durable supervisor wake that survives restart', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-terminal-wake-'));
  const file = path.join(directory, 'control-plane.sqlite');
...
test('supervisor projection survives database restart with stable durable lineage', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-supervisor-restart-'));
  const file = path.join(directory, 'control-plane.sqlite');
  let db = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
...
test('supervisor wake queue is durable, idempotent and atomically drained', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-wake-'));
  const file = path.join(directory, 'control-plane.sqlite');
  let db = openDatabase(file, { environment: 'test', env: { NODE_ENV: 'test' } });
...
test('maintenance candidates survive database restart with immutable plan binding', () => {
  ...
  assert.throws(() => first.attachPlan(created.candidate.candidateId, 'missing-plan'), (error: unknown) => error instanceof ForgeFlowError && error.code === 'PLAN_NOT_FOUND');
  ...
  assert.throws(() => second.attachPlan(recovered.candidate.candidateId, 'plan-2'), ForgeFlowError);
  db.close();
});
...
test('supervisor runtime releases lease after model failure and can be retried', async () => {
  ...
  const runtime = new SupervisorRuntime(db, seeded.repos.supervisors, scheduler, host, new SupervisorActionExecutor(seeded.repos.actions, seeded.repos.decisions, {}), { decide: async () => { throw new ForgeFlowError('SUPERVISOR_MODEL_UNAVAILABLE'); } }, 'runtime-failure-owner');
  const result = await runtime.runOnce();
...
test('supervisor HTTP model client bounds requests and rejects provider failures', async () => {
  ...
  await assert.rejects(() => failing.decide({ conversationId: 'c', supervisorId: 's', planId: 'p', projection: { cursor: 0, digest: 'd' } as never }), (error: unknown) => error instanceof ForgeFlowError && error.code === 'SUPERVISOR_MODEL_UNAVAILABLE');
});
...
test('adapters deduplicate candidates, invalidate force-pushed PRs and gate resources', () => {
  ...
  assert.throws(() => github.prepareMerge(change, { currentHeadSha: 'head-b', reviewId: 'review', checksPassed: true, autoMergeAuthorized: true }), ForgeFlowError);
  ...
  assert.throws(() => jules.correlate(request, { ...result, baseRevision: 'other' }), ForgeFlowError);
  assert.throws(() => new AntiGravityReadinessAdapter().requireReady(), ForgeFlowError);
});
