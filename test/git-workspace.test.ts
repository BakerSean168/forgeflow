...
function repository(root: string): {
  ...
  git(repositoryPath, ['config', 'user.name', 'ForgeFlow Test']);
  git(repositoryPath, ['config', 'user.email', 'forgeflow-test@local']);
...
function fixture() {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-git-workspace-'));
...
function configureWriter(workspace: WorkspaceDescriptor): void {
  git(workspace.hostPath, ['config', 'user.name', 'ForgeFlow Worker']);
  git(workspace.hostPath, ['config', 'user.email', 'forgeflow-worker@local']);
}
...
test('LocalGitWorkspace fails closed before provisioning when managed storage is below threshold', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_CAPACITY_LOW',
  );
  assert.equal(
    fs.existsSync(path.join(value.managedRoot, 'forgeflow', 'executions', 'exec-capacity-low')),
    false,
  );
});
...
test('LocalGitWorkspace observes allowed repositories and rejects roots and symlinks', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_REPOSITORY_NOT_ALLOWED',
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_REPOSITORY_INVALID',
  );
});
...
test('LocalGitWorkspace provisions an exact revision idempotently with durable provenance', async () => {
  ...
  assert.equal(first.executionPath, '/workspace/forgeflow/executions/exec-provision/repo');
  assert.equal(first.sourceRepositoryPath, value.repositoryPath);
  assert.ok(
    fs.existsSync(
      path.join(value.managedRoot, 'forgeflow', 'executions', 'exec-provision', 'workspace.json'),
    ),
  );
  await assert.rejects(
    () =>
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_PROVENANCE_MISMATCH',
  );
});
...
test('LocalGitWorkspace implementation verification rejects no-op, dirty and mismatched evidence then accepts exact committed work', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_IMPLEMENTATION_NOOP',
  );
  write(
  ...
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_DIRTY',
  );
  ...
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_IMPLEMENTATION_OUTCOME_INVALID',
  );
  write(workspace.evidenceHostPath, JSON.stringify(implementationEvidence(workspace, 'wrong-sha')));
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_IMPLEMENTATION_EVIDENCE_MISMATCH',
  );
  write(
    workspace.evidenceHostPath,
    JSON.stringify(
  ...
  await assert.rejects(
    () => value.adapter.verifyImplementation(workspace),
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_IMPLEMENTATION_TEST_GATE_FAILED',
  );
  write(
    workspace.evidenceHostPath,
...
test('LocalGitWorkspace promotes controller-staged repository evidence and rejects unsafe staging files', async () => {
  ...
  assert.match(
    ...
    /\?\? \.forgeflow-completion-evidence\.json/,
  );
  const verified = await value.adapter.verifyImplementation(workspace);
  assert.equal(verified.headRevision, resultRevision);
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_REPOSITORY_EVIDENCE_TRACKED',
  );
  ...
  write(
    '/tmp/forgeflow-evidence-target.json',
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_REPOSITORY_EVIDENCE_INVALID',
  );
  fs.rmSync('/tmp/forgeflow-evidence-target.json', { force: true });
});
...
test('LocalGitWorkspace provisions independent exact-SHA review snapshots and validates verdict evidence', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_REVIEW_EVIDENCE_MISMATCH',
  );
  ...
  await assert.rejects(
    () => value.adapter.verifyReview(review, reviewedSha),
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_EVIDENCE_INVALID',
  );
...
test('LocalGitWorkspace integration rejects stale, dirty and non-descendant candidates and fast-forwards exact accepted revision', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_INTEGRATION_STALE_HEAD',
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_INTEGRATION_DIRTY',
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_ACCEPTED_REVISION_NOT_DESCENDANT',
  );
  ...
  const integrationLock = path.join(gitDirectory, 'forgeflow-integration.lock');
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ForgeFlowError && error.code === 'WORKSPACE_INTEGRATION_LOCKED',
  );
...
test('LocalGitWorkspace integration aligns initialized submodules and replays a post-fast-forward submodule checkout crash safely', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ForgeFlowError && error.code === 'WORKSPACE_INTEGRATION_SUBMODULE_DIRTY',
  );
...
test('LocalGitWorkspace keeps managed parents controller-owned but traversable by the worker', async () => {
  ...
  const forgeFlowRoot = path.join(value.managedRoot, 'forgeflow');
  const executionsRoot = path.join(forgeFlowRoot, 'executions');
  const executionDirectory = path.dirname(workspace.hostPath);
  for (const directory of [forgeFlowRoot, executionsRoot]) {
