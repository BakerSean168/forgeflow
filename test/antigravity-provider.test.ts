...
function fixture(mode: 'implementation' | 'review' | 'wait' = 'implementation') {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-antigravity-'));
  const executionRoot = path.join(root, 'workspaces', 'forgeflow', 'executions', 'exec-ant');
...
function input(
  ...
  return {
    ...
    workspace: {
      ...
      executionPath: '/workspace/forgeflow/executions/exec-ant/repo',
      evidenceHostPath: value.evidence,
      evidenceExecutionPath: '/workspace/forgeflow/executions/exec-ant/completion-evidence.json',
