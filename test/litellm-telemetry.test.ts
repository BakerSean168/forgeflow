...
test('ForgeFlow LiteLLM telemetry uses exact end_user correlation and reports physical provider routes', async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'forgeflow-litellm-telemetry-'));
