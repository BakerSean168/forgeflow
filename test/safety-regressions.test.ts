...
test('invalid graph input fails before creating or superseding a graph version', () => {
  ...
  assert.throws(() => kernel.ensureReadyGraph(plan.planId, [{
    ...
    acceptanceCriteria: [],
  }]), (error: unknown) => error instanceof ForgeFlowError && error.code === 'GRAPH_DEPENDENCY_NOT_FOUND');
  assert.equal(repos.plans.getActiveGraphVersion(plan.planId), undefined);
  db.close();
});

test('durable supervisor observation cursor suppresses a second decision for the same projection', async () => {
  ...
  assert.equal((await executor.execute(make('cursor-action-1'), projection)).status, 'SUCCEEDED');
  assert.equal(repos.supervisors.getById(supervisor.supervisorId).observationCursor, projection.cursor);
  const second = await executor.execute(make('cursor-action-2'), projection);
