...
import type { PlanWorktree } from '../../core/domain/worktree.js';
...
function readJson(file: string, code: string): Record<string, unknown> {
  ...
  try {
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw new ForgeFlowError(code);
...
export class LiteralWorktreeWorkspaceAdapter implements WorkspaceProviderPort {
  ...
  async deliveryWorkspace(planId: string): Promise<string | undefined> {
    ...
    if (plan.planId === root.planId) {
      ...
      if (!integration) throw new ForgeFlowError('WORKTREE_INTEGRATION_MISSING');
    ...
    if (candidates.length !== 1) throw new ForgeFlowError('WORKTREE_DELIVERY_CANDIDATE_AMBIGUOUS');
  ...
  async verifyReview(
    ...
    const record = this.repositories.planWorktrees.findByPath(descriptor.hostPath);
    if (!record || record.role !== 'REVIEW') throw new ForgeFlowError('WORKTREE_REVIEW_REGISTRY_MISSING');
  ...
  private async git(cwd: string, args: string[], allowFailure = false): Promise<string> {
    try {
      ...
      throw new ForgeFlowError('WORKSPACE_GIT_FAILED', 'Literal worktree Git command failed.', error);
    }
  }
