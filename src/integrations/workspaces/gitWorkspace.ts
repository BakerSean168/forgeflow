...
import { assertSafeEventPayload } from '../../core/domain/events.js';
import { REPOSITORY_COMPLETION_EVIDENCE_FILE } from '../../core/orchestration/contracts.js';
import type {
  CompletionEvidence,
...
function executionComponent(value: string): string {
  ...
  if (
    ...
    throw new ForgeFlowError('WORKSPACE_EXECUTION_ID_INVALID');
  }
  return component;
}
...
function requiredString(value: unknown, code: string, maximum = 8_000): string {
  if (typeof value !== 'string' || !value.trim() || value.length > maximum) throw new ForgeFlowError(code);
  return value;
}
...
function decodeTest(value: unknown): TestCommandEvidence {
  ...
  if (status !== 'PASS' && status !== 'FAIL' && status !== 'SKIP')
    throw new ForgeFlowError('WORKSPACE_TEST_EVIDENCE_INVALID');
...
export function decodeImplementationEvidence(value: unknown): ImplementationCompletionEvidence {
  ...
  if (root.version !== 1 || (root.phase !== 'IMPLEMENT' && root.phase !== 'IMPLEMENT_FIX'))
    throw new ForgeFlowError('WORKSPACE_IMPLEMENTATION_EVIDENCE_INVALID');
  if (!Array.isArray(root.tests)) throw new ForgeFlowError('WORKSPACE_TEST_EVIDENCE_INVALID');
  const outcome = root.outcome ?? 'CHANGED';
  if (outcome !== 'CHANGED' && outcome !== 'SATISFIED')
    throw new ForgeFlowError('WORKSPACE_IMPLEMENTATION_OUTCOME_INVALID');
...
export function decodeReviewEvidence(value: unknown): ReviewCompletionEvidence {
  ...
  if (root.version !== 1 || root.phase !== 'REVIEW')
    throw new ForgeFlowError('WORKSPACE_REVIEW_EVIDENCE_INVALID');
  if (root.verdict !== 'PASS' && root.verdict !== 'FAIL' && root.verdict !== 'INVALID')
    throw new ForgeFlowError('WORKSPACE_REVIEW_VERDICT_INVALID');
  if (
    !Array.isArray(root.findings) ||
    !root.findings.every((item) => typeof item === 'string' && item.length <= 8_000)
  ) {
    throw new ForgeFlowError('WORKSPACE_REVIEW_FINDINGS_INVALID');
  }
  if (!Array.isArray(root.checks)) throw new ForgeFlowError('WORKSPACE_REVIEW_CHECKS_INVALID');
  return {
    version: 1,
    executionId: requiredString(
      root.executionId,
...
function descriptorFrom(value: unknown): StoredWorkspaceDescriptor {
  ...
  if (root.version !== 1) throw new ForgeFlowError('WORKSPACE_DESCRIPTOR_VERSION_INVALID');
...
export function assertImplementationEvidenceGate(
  ...
  if (
    ...
    throw new ForgeFlowError('WORKSPACE_IMPLEMENTATION_EVIDENCE_MISMATCH');
  ...
  if (expectedRevision === descriptor.sourceRevision && !satisfiedWithoutChange)
    throw new ForgeFlowError('WORKSPACE_IMPLEMENTATION_NOOP');
  ...
  if (
    ...
    throw new ForgeFlowError('WORKSPACE_IMPLEMENTATION_TEST_GATE_FAILED');
}
...
export function assertReviewEvidenceGate(
  ...
  if (evidence.executionId !== descriptor.executionId || evidence.reviewedSha !== reviewedSha)
    throw new ForgeFlowError('WORKSPACE_REVIEW_EVIDENCE_MISMATCH');
  if (evidence.verdict === 'PASS') {
    if (
      ...
      throw new ForgeFlowError('WORKSPACE_REVIEW_CHECK_GATE_FAILED');
  } else if (
    evidence.findings.length === 0 &&
    !evidence.checks.some((item) => item.status === 'FAIL')
  )
    throw new ForgeFlowError('WORKSPACE_REVIEW_FINDINGS_REQUIRED');
}
...
export class LocalGitWorkspaceAdapter implements WorkspaceProviderPort {
  ...
  async provision(input: WorkspaceProvisionInput): Promise<WorkspaceDescriptor> {
    ...
    if (fs.existsSync(executionDirectory)) {
      if (!fs.existsSync(descriptorPath)) throw new ForgeFlowError('WORKSPACE_PROVISION_INCOMPLETE');
      ...
      for (const [key, value] of Object.entries(expected)) {
        if (existing[key as keyof typeof existing] !== value)
          throw new ForgeFlowError('WORKSPACE_PROVENANCE_MISMATCH');
      }
      ...
      if (
        ...
        throw new ForgeFlowError('WORKSPACE_SOURCE_REVISION_MISSING');
      }
      return existing;
    }
    ...
    if (input.sourceWorkspace) {
      ...
      if (source.sourceRepositoryPath !== repositoryRoot)
        throw new ForgeFlowError('WORKSPACE_SOURCE_REPOSITORY_MISMATCH');
      if (
  ...
  storageStatus(): WorkspaceStorageStatus {
    ...
    if (
      ...
      throw new ForgeFlowError('WORKSPACE_CAPACITY_PROBE_FAILED');
    return {
      totalBytes,
      freeBytes,
  ...
  async verifyImplementation(workspace: WorkspaceDescriptor): Promise<WorkspaceCompletionSnapshot> {
    ...
    if (!clean) throw new ForgeFlowError('WORKSPACE_DIRTY');
  ...
  async verifyReview(
    ...
    if (headRevision !== reviewedSha || descriptor.sourceRevision !== reviewedSha)
      throw new ForgeFlowError('WORKSPACE_REVIEW_SHA_MISMATCH');
  ...
  async integrateAcceptedRevision(input: {
    ...
    if (!candidateClean || candidateHead !== input.acceptedRevision)
      throw new ForgeFlowError('WORKSPACE_ACCEPTED_REVISION_NOT_PINNED');
    if (
      ...
      throw new ForgeFlowError('WORKSPACE_ACCEPTED_REVISION_NOT_DESCENDANT');
    }
    ...
    const lockPath = path.join(gitDirectory, 'forgeflow-integration.lock');
    ...
    try {
      ...
      if (code === 'EEXIST') throw new ForgeFlowError('WORKSPACE_INTEGRATION_LOCKED');
    ...
    const transferBundle = path.join(
      gitDirectory,
      'forgeflow-integration-' + randomUUID() + '.bundle',
    );
    try {
      ...
      if (
        ...
        throw new ForgeFlowError('WORKSPACE_INTEGRATION_STALE_HEAD');
      ...
      if ((current === input.expectedRevision || alreadyContained) && repositoryStatus.length !== 0)
        throw new ForgeFlowError('WORKSPACE_INTEGRATION_DIRTY');
      ...
      if (after !== input.acceptedRevision)
        throw new ForgeFlowError('WORKSPACE_INTEGRATION_RESULT_MISMATCH');
      if (
  ...
  private safeSubmodulePath(repositoryRoot: string, relativePath: string): string {
    ...
    if (
      ...
      throw new ForgeFlowError('WORKSPACE_SUBMODULE_PATH_INVALID');
    const targetPath = path.resolve(repositoryRoot, normalized);
    if (!inside(targetPath, repositoryRoot)) throw new ForgeFlowError('WORKSPACE_SUBMODULE_PATH_INVALID');
    return targetPath;
  }
  ...
  private async initializedSubmoduleState(
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink())
      throw new ForgeFlowError('WORKSPACE_INTEGRATION_SUBMODULE_INVALID');
    ...
    if (!gitDirectory) throw new ForgeFlowError('WORKSPACE_INTEGRATION_SUBMODULE_INVALID');
  ...
  private async preflightInitializedSubmodules(
    ...
    if (depth > MAX_SUBMODULE_DEPTH) throw new ForgeFlowError('WORKSPACE_SUBMODULE_DEPTH_EXCEEDED');
    ...
    for (const entry of this.gitlinkEntries(tree)) {
      if (state.count >= MAX_SUBMODULES) throw new ForgeFlowError('WORKSPACE_SUBMODULE_LIMIT_EXCEEDED');
      state.count += 1;
      ...
      if (status.length !== 0) throw new ForgeFlowError('WORKSPACE_INTEGRATION_SUBMODULE_DIRTY');
      if (
        !(await this.gitSucceeds(
          initialized.path,
        ...
        throw new ForgeFlowError('WORKSPACE_INTEGRATION_SUBMODULE_REVISION_MISSING');
  ...
  private async assertReplayDirtinessRepairable(
    ...
    if (staged.length !== 0) throw new ForgeFlowError('WORKSPACE_INTEGRATION_DIRTY');
    const untracked = await this.git(
      repositoryRoot,
      ['ls-files', '--others', '--exclude-standard', '-z'],
      identity,
    );
    if (untracked.length !== 0) throw new ForgeFlowError('WORKSPACE_INTEGRATION_DIRTY');
    const changed = (
      await this.git(
        repositoryRoot,
    ...
    for (const entry of entries) {
      ...
      for (const candidatePath of candidatePaths) {
        ...
        if (!sourceStat.isDirectory() || sourceStat.isSymbolicLink())
          throw new ForgeFlowError('WORKSPACE_SUBMODULE_SOURCE_INVALID');
      ...
      if (!objectSource) throw new ForgeFlowError('WORKSPACE_SUBMODULE_SOURCE_REVISION_MISMATCH');
      ...
      if (existing) {
        if (
          ...
          throw new ForgeFlowError('WORKSPACE_SUBMODULE_TARGET_NOT_EMPTY');
      ...
      if (!bundleRef) throw new ForgeFlowError('WORKSPACE_SUBMODULE_SOURCE_REVISION_MISMATCH');
      const bundle = path.join(
        bundleRoot,
        '.forgeflow-submodule-' + state.count + '-' + randomUUID() + '.bundle',
      );
      try {
  ...
  private ensureManagedExecutionParents(): string {
    ...
    const directories = [
      ...
      path.join(this.managedHostRoot, 'forgeflow', 'executions'),
    ];
    for (const directory of directories) {
      ...
      if (
        ...
        if (managerUid !== 0) throw new ForgeFlowError('WORKSPACE_MANAGED_PARENT_OWNER_INVALID');
  ...
  private canonicalDirectory(value: string, code: string): string {
    ...
    if (!stat?.isDirectory() || stat.isSymbolicLink()) throw new ForgeFlowError(code);
    const real = fs.realpathSync(lexical);
    if (real !== lexical) throw new ForgeFlowError(code);
    return real;
  }
  ...
  private async repositoryRoot(repositoryPath: string): Promise<string> {
    ...
    if (!this.allowedRoots.some((root) => inside(lexical, root)))
      throw new ForgeFlowError('WORKSPACE_REPOSITORY_NOT_ALLOWED');
  ...
  private validateWorkspace(value: WorkspaceDescriptor): WorkspaceDescriptor {
    ...
    if (
      ...
      throw new ForgeFlowError('WORKSPACE_SOURCE_REPOSITORY_INVALID');
    }
    const executionId = executionComponent(descriptor.executionId);
    const expectedDirectory = path.join(this.managedHostRoot, 'forgeflow', 'executions', executionId);
    if (
    ...
    if (
      ...
      path.posix.join(this.executionRoot, 'forgeflow', 'executions', executionId, 'repo')
    )
      throw new ForgeFlowError('WORKSPACE_PROVENANCE_MISMATCH');
    if (
    ...
    if (JSON.stringify(stored) !== JSON.stringify(descriptor))
      throw new ForgeFlowError('WORKSPACE_DESCRIPTOR_MISMATCH');
    return descriptor;
  }
  ...
  private async promoteRepositoryEvidence(
    ...
    if (existingStat) {
      if (
        ...
        throw new ForgeFlowError('WORKSPACE_EVIDENCE_INVALID');
      ...
      if (existingValid) {
        if (existingCanonical !== JSON.stringify(evidence))
          throw new ForgeFlowError('WORKSPACE_EVIDENCE_AMBIGUOUS');
    ...
    try {
      ...
      throw new ForgeFlowError('WORKSPACE_EVIDENCE_PROMOTION_FAILED');
    }
  }
  ...
  private readDescriptor(file: string): StoredWorkspaceDescriptor {
    ...
    if (!stat?.isFile() || stat.isSymbolicLink() || stat.size > 64_000)
      throw new ForgeFlowError('WORKSPACE_DESCRIPTOR_INVALID');
    try {
  ...
  private async runGit(args: string[], identity?: { uid: number; gid: number }): Promise<string> {
    try {
      ...
      throw new ForgeFlowError('WORKSPACE_GIT_COMMAND_FAILED');
    }
  }
}
