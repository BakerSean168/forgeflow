import type { SupervisorProjection } from '../../core/supervisor/projection.js';
import type { SupervisorConversation, SupervisorConversationHost } from '../../core/supervisor/host.js';
...
export class HttpOpenHandsSupervisorClient implements OpenHandsSupervisorClient {
  ...
  async createSupervisorConversation(input: { supervisorId: string; planId: string; projectionDigest: string; boundedInstruction: string }): Promise<SupervisorConversation> {
    const response = await this.fetchImpl(this.url('/api/conversations'), {
      ...
      body: JSON.stringify({
        workspace: { kind: 'LocalWorkspace', working_dir: '/tmp/forgeflow-supervisor' },
...
function boundedInstruction(projection: SupervisorProjection): string {
  return [
    'You are the ForgeFlow read-only plan supervisor.',
...
export class OpenHandsSupervisorAdapter implements SupervisorConversationHost {
