...
import { DuplicateKeyError, ForgeFlowError, failClosed } from '../../core/domain/errors.js';
import { assertCurrentSchema, openDatabase, withTransaction } from '../../core/persistence/database.js';
...
function decodeResult(row: JulesRow): JulesTaskResult {
  try {
    ...
    if (result.sessionId !== row.session_id || result.repository !== row.repository || result.baseRevision !== row.base_revision) {
      throw new ForgeFlowError('CORRUPTED_JULES_PROVENANCE');
    }
    return result;
  } catch (error) {
    if (error instanceof ForgeFlowError) throw error;
    throw new ForgeFlowError('CORRUPTED_JULES_RESULT', 'Stored Jules result is invalid.', error);
  }
}
...
function assertMonotonicUpdate(current: JulesTaskResult, next: JulesTaskResult): void {
  if (current.sessionId !== next.sessionId) throw new ForgeFlowError('JULES_SESSION_IMMUTABLE');
  if (current.repository !== next.repository || current.baseRevision !== next.baseRevision) throw new ForgeFlowError('JULES_PROVENANCE_MISMATCH');
  if (current.status !== 'RUNNING') throw new ForgeFlowError('JULES_RESULT_IMMUTABLE');
  if (current.headRevision && current.headRevision !== next.headRevision) throw new ForgeFlowError('JULES_HEAD_REVISION_IMMUTABLE');
  if (current.pullRequestId && current.pullRequestId !== next.pullRequestId) throw new ForgeFlowError('JULES_PULL_REQUEST_IMMUTABLE');
}
...
export class JulesAdapter {
  constructor(
    ...
    readonly db: DatabaseSync = openDatabase(':memory:', { environment: 'test', env: { NODE_ENV: 'test' } }),
  ) {
  ...
  submit(input: JulesTaskRequest): JulesTaskResult {
    ...
    if (durable) {
      ...
      if (existing.repository !== input.repository || existing.baseRevision !== input.baseRevision) throw new ForgeFlowError('JULES_PROVENANCE_MISMATCH');
      return existing;
    }
    if (!this.client) throw new ForgeFlowError('JULES_CLIENT_UNAVAILABLE');
  ...
  getResult(sessionId: string): JulesTaskResult {
    ...
    if (!durable) throw new ForgeFlowError('JULES_SESSION_NOT_FOUND');
  ...
  private findByIdempotencyKey(key: string): JulesRow | undefined {
    return this.db.prepare('SELECT * FROM jules_sessions WHERE idempotency_key=?').get(key) as JulesRow | undefined;
  }

  private findBySessionId(sessionId: string): JulesRow | undefined {
    return this.db.prepare('SELECT * FROM jules_sessions WHERE session_id=?').get(sessionId) as JulesRow | undefined;
  }

  private persist(request: JulesTaskRequest, result: JulesTaskResult): JulesTaskResult {
    validateResult(result);
    if (result.repository !== request.repository || result.baseRevision !== request.baseRevision) throw new ForgeFlowError('JULES_PROVENANCE_MISMATCH');
    return withTransaction(this.db, () => {
      ...
      if (byKey) {
        ...
        const update = this.db.prepare('UPDATE jules_sessions SET result=?,updated_at=? WHERE idempotency_key=? AND session_id=? AND result=?').run(
          encoded, updatedAt, request.idempotencyKey, current.sessionId, byKey.result,
        );
        if (Number(update.changes) !== 1) throw new ForgeFlowError('JULES_RESULT_STALE');
      ...
      const encoded = encodedResult(result);
      this.db.prepare('INSERT INTO jules_sessions(idempotency_key,session_id,repository,base_revision,result,updated_at) VALUES(?,?,?,?,?,?)').run(
