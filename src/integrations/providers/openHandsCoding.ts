...
function tagValue(value: string): string {
  ...
  if (!normalized) throw new ForgeFlowError('OPENHANDS_TAG_INVALID');
  return normalized;
}

function executionTag(executionId: string): string {
  const normalized = executionId.replace(/[^A-Za-z0-9]/g, '').slice(0, 128);
  if (!normalized) throw new ForgeFlowError('OPENHANDS_EXECUTION_TAG_INVALID');
  return normalized;
}
...
function phasePrompt(
  ...
  const text = [
    'ForgeFlow execution phase: ' + input.phase,
  ...
  if (Buffer.byteLength(text, 'utf8') > MAX_INSTRUCTION_TEXT)
    throw new ForgeFlowError('OPENHANDS_INSTRUCTION_TOO_LARGE');
  return text;
}
...
abstract class OpenHandsProviderBase implements ExecutionProviderPort {
  ...
  async recover(input: ProviderRecoveryInput): Promise<ProviderSessionSnapshot | undefined> {
    ...
    for (let page = 0; page < MAX_RECOVERY_PAGES; page += 1) {
      ...
      for (const item of items) {
        ...
        if (matches.size > 1) throw new ForgeFlowError('OPENHANDS_EXECUTION_DUPLICATE');
      }
  ...
  protected conversationSecrets(input: ProviderLaunchInput): JsonRecord {
    ...
    const literal = input.workspace.executionPath.startsWith('/workspace/forgeflow/plans/');
  ...
  protected async createConversation(
    ...
    const payload = {
      ...
      observability_tags: ['forgeflow', input.phase.toLowerCase()],
    };
  ...
  private async findReplacement(
    ...
    for (let page = 0; page < MAX_RECOVERY_PAGES; page += 1) {
      ...
      for (const item of items) {
        ...
        matches.set(id, conversation);
        if (matches.size > 1) throw new ForgeFlowError('OPENHANDS_REPLACEMENT_DUPLICATE');
      }
      const next = boundedText(payload.next_page_id, 2_000);
      if (!next) break;
      pageId = next;
  ...
  private async ensureRunning(
    ...
    try {
      ...
      if (!(error instanceof ForgeFlowError) || error.code !== 'OPENHANDS_HTTP_409') throw error;
  ...
  protected validatePhase(phase: ExecutionPhase): void {
    if (this.mode === 'REVIEW' && phase !== 'REVIEW')
      throw new ForgeFlowError('OPENHANDS_REVIEW_PHASE_REQUIRED');
    if (this.mode === 'IMPLEMENTATION' && phase === 'REVIEW')
      throw new ForgeFlowError('OPENHANDS_IMPLEMENTATION_PHASE_REQUIRED');
  }
  ...
  private validateConversation(
    ...
    if (tags.execution !== undefined && tags.execution !== executionTag(input.executionId))
      throw new ForgeFlowError('OPENHANDS_EXECUTION_PROVENANCE_MISMATCH');
    if (
      input.projectKey &&
    ...
    if (
      ...
      throw new ForgeFlowError('OPENHANDS_WORKSPACE_PROVENANCE_MISMATCH');
    }
  }
  ...
  private async snapshot(
    ...
    if (TERMINAL_STATUSES.has(status)) {
      try {
        ...
        if (!(error instanceof ForgeFlowError) || !error.code.startsWith('OPENHANDS_HTTP_')) throw error;
      }
    }
  ...
  private async json(path: string, init: RequestInit = {}): Promise<JsonRecord> {
    ...
    try {
      ...
      throw new ForgeFlowError('OPENHANDS_RESPONSE_INVALID', 'OpenHands returned invalid JSON.');
    }
    if (payload === null || typeof payload !== 'object' || Array.isArray(payload))
      throw new ForgeFlowError('OPENHANDS_RESPONSE_INVALID');
  ...
  private async request(path: string, init: RequestInit = {}): Promise<Response> {
    try {
      ...
      if (error instanceof ForgeFlowError) {
        throw new ForgeFlowError(error.code, this.sanitize(error.message, MAX_ERROR_TEXT) ?? error.code);
      }
      if (error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError'))
        throw new ForgeFlowError('OPENHANDS_TIMEOUT');
      throw new ForgeFlowError('OPENHANDS_UNAVAILABLE', 'OpenHands request failed.');
    }
  }
}
...
function literalGitProvenance(
  ...
  if (!input.workspace.executionPath.startsWith('/workspace/forgeflow/plans/')) return undefined;
...
class OpenHandsModelNativeAcpProvider extends OpenHandsProviderBase {
  ...
  private runtimeProbeSecrets(input: ProviderLaunchInput): JsonRecord {
    ...
    for (const key of [
      'FORGEFLOW_IMPLEMENTATION_EVIDENCE_PATH',
      'FORGEFLOW_REVIEW_EVIDENCE_PATH',
      'FORGEFLOW_SOURCE_SHA',
      'FORGEFLOW_REVIEWED_SHA',
      'FORGEFLOW_IMPLEMENTATION_PHASE',
    ])
      delete secrets[key];
    if (this.driver === 'codex' || this.driver === 'claude') {
      secrets.FORGEFLOW_HEADLESS_ROLE = { kind: 'StaticSecret', value: 'planner' };
      secrets.FORGEFLOW_HEADLESS_REASONING_EFFORT = { kind: 'StaticSecret', value: 'low' };
      secrets.FORGEFLOW_HEADLESS_TIMEOUT_SECONDS = { kind: 'StaticSecret', value: '60' };
    }
    return secrets;
  }
  ...
  protected override conversationSecrets(input: ProviderLaunchInput): JsonRecord {
    const secrets: JsonRecord = {
      ...
      FORGEFLOW_AGENT_BACKEND: { kind: 'StaticSecret', value: this.driver + '-acp' },
      FORGEFLOW_AGENT_TRANSPORT: { kind: 'StaticSecret', value: this.transport },
      FORGEFLOW_AGENT_MODEL: { kind: 'StaticSecret', value: this.selectedModel },
      FORGEFLOW_WORKSPACE_ROOT: {
        kind: 'StaticSecret',
        value: this.acp.workspaceRoot,
      },
      FORGEFLOW_EXECUTION_ID: { kind: 'StaticSecret', value: input.executionId },
      FORGEFLOW_WORKSPACE_REF: {
        kind: 'StaticSecret',
    ...
    if (gitProvenance) {
      secrets.FORGEFLOW_EXPECTED_GIT_COMMON_DIR = {
        kind: 'StaticSecret',
        value: gitProvenance.commonDir,
      };
      secrets.FORGEFLOW_EXPECTED_WORKTREE_GIT_FILE = {
        kind: 'StaticSecret',
    ...
    if (this.driver === 'codex' || this.driver === 'claude') {
      secrets.FORGEFLOW_HEADLESS_DRIVER = { kind: 'StaticSecret', value: this.driver };
      secrets.FORGEFLOW_HEADLESS_ROLE = {
        kind: 'StaticSecret',
        value: this.mode === 'IMPLEMENTATION' ? 'worker' : 'review',
      };
      secrets.FORGEFLOW_HEADLESS_TRANSPORT = {
        kind: 'StaticSecret',
        value: this.transport,
      };
      secrets.FORGEFLOW_HEADLESS_MODEL = {
        kind: 'StaticSecret',
        value: this.selectedModel,
      };
      secrets.FORGEFLOW_HEADLESS_REASONING_EFFORT = {
        kind: 'StaticSecret',
        value: this.acp.reasoningEffort,
      };
      secrets.FORGEFLOW_HEADLESS_TIMEOUT_SECONDS = {
        kind: 'StaticSecret',
        value: String(this.acp.promptTimeoutSeconds),
      };
      if (this.binary) {
        secrets[this.driver === 'codex' ? 'FORGEFLOW_CODEX_BIN' : 'FORGEFLOW_CLAUDE_BIN'] = {
          kind: 'StaticSecret',
          value: this.binary,
        };
      }
    ...
    if (this.transport === 'litellm-managed') {
      secrets.FORGEFLOW_LITELLM_API_KEY = {
        kind: 'StaticSecret',
        value: this.options.liteLlmApiKey,
      };
      secrets.FORGEFLOW_LITELLM_BASE_URL = {
        kind: 'StaticSecret',
      ...
      secrets.FORGEFLOW_CODEX_AUTH_HOME = {
        kind: 'StaticSecret',
        value: this.authHome,
      };
    }
    ...
    if (this.mode === 'IMPLEMENTATION') {
      ...
      secrets.GIT_AUTHOR_NAME = { kind: 'StaticSecret', value: 'ForgeFlow' };
      secrets.GIT_AUTHOR_EMAIL = { kind: 'StaticSecret', value: 'forgeflow@localhost' };
      secrets.GIT_COMMITTER_NAME = { kind: 'StaticSecret', value: 'ForgeFlow' };
      secrets.GIT_COMMITTER_EMAIL = { kind: 'StaticSecret', value: 'forgeflow@localhost' };
      secrets.FORGEFLOW_IMPLEMENTATION_EVIDENCE_PATH = {
        kind: 'StaticSecret',
        value: this.evidencePath(input),
      };
      secrets.FORGEFLOW_SOURCE_SHA = { kind: 'StaticSecret', value: input.sourceRevision };
      secrets.FORGEFLOW_IMPLEMENTATION_PHASE = { kind: 'StaticSecret', value: input.phase };
    } else {
      secrets.FORGEFLOW_REVIEW_EVIDENCE_PATH = {
        kind: 'StaticSecret',
        value: this.evidencePath(input),
      };
      secrets.FORGEFLOW_REVIEWED_SHA = { kind: 'StaticSecret', value: input.sourceRevision };
    }
    secrets.FORGEFLOW_EXECUTION_ID = { kind: 'StaticSecret', value: input.executionId };
    return secrets;
  }
}
...
const HEADLESS_COMMAND = [
  '/usr/local/bin/node',
  '/opt/forgeflow-tools/headless_review_acp.mjs',
];
const HARNESS_LAUNCHER = '/opt/forgeflow-tools/harness_agent_launcher.sh';
...
export function createOpenHandsProviderForSelection(
  ...
  switch (selection.backend) {
    ...
    default:
      throw new ForgeFlowError('OPENHANDS_BACKEND_UNSUPPORTED');
  }
}
