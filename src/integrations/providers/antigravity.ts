...
import type { ResourceObservation } from '../../core/domain/resource.js';
import type {
...
abstract class AntigravityProviderBase implements ExecutionProviderPort {
  ...
  async launch(input: ProviderLaunchInput): Promise<ProviderSessionSnapshot> {
    ...
    if (this.health() !== 'OK') throw new ForgeFlowError('ANTIGRAVITY_UNAVAILABLE');

    const workspace = path.resolve(input.workspace.hostPath);
    if (!inside(workspace, this.workspaceHostRoot))
      throw new ForgeFlowError('ANTIGRAVITY_WORKSPACE_NOT_ALLOWED');
    if (!fs.statSync(workspace, { throwIfNoEntry: false })?.isDirectory())
      throw new ForgeFlowError('ANTIGRAVITY_WORKSPACE_NOT_FOUND');
    ...
    if (!child.pid || !child.stdin) throw new ForgeFlowError('ANTIGRAVITY_PROCESS_START_FAILED');
    const meta: AntigravityExecutionMeta = {
      version: 1,
      executionId,
      providerSessionId,
      ...
      if (status === 'SUCCESS') {
        try {
          ...
          return this.snapshot(
            ...
            error instanceof ForgeFlowError ? error.code : 'ANTIGRAVITY_EVIDENCE_FAILED',
            true,
          );
        }
      }
  ...
  async cancel(providerSessionId: string): Promise<ProviderSessionSnapshot> {
    ...
    if (meta.systemdUnit) {
      try {
        ...
        // A concurrently completed unit is already cancelled for ForgeFlow purposes.
      }
  ...
  private validateInput(input: ProviderLaunchInput): void {
    if (this.role === 'REVIEW' && input.phase !== 'REVIEW')
      throw new ForgeFlowError('ANTIGRAVITY_REVIEW_PHASE_REQUIRED');
    if (this.role === 'IMPLEMENTATION' && input.phase === 'REVIEW')
      throw new ForgeFlowError('ANTIGRAVITY_IMPLEMENTATION_PHASE_REQUIRED');
  ...
  private prompt(input: ProviderLaunchInput, workspace: string): string {
    const acceptance = input.acceptanceCriteria.length
      ...
      : [];
    const common = [
      'ForgeFlow provider-native Antigravity execution.',
  ...
  private materializeEvidence(
    ...
    if (!clean) throw new ForgeFlowError('ANTIGRAVITY_WORKSPACE_DIRTY');
    ...
    if (checks.some((item) => item.status === 'FAIL'))
      throw new ForgeFlowError('ANTIGRAVITY_CHECK_FAILED');
    ...
    if (verdict !== 'PASS' && verdict !== 'FAIL' && verdict !== 'INVALID')
      throw new ForgeFlowError('ANTIGRAVITY_REVIEW_VERDICT_INVALID');
  ...
  private systemdUnit(executionId: string): string {
    ...
    if (!template.includes('%i')) throw new ForgeFlowError('ANTIGRAVITY_SYSTEMD_TEMPLATE_INVALID');
    const unit = template.replaceAll('%i', safeId(executionId));
    if (!/^[A-Za-z0-9_.@-]+\.service$/.test(unit))
      throw new ForgeFlowError('ANTIGRAVITY_SYSTEMD_UNIT_INVALID');
    return unit;
  }
  ...
  private executionId(providerSessionId: string): string {
    if (!providerSessionId.startsWith('antigravity:'))
      throw new ForgeFlowError('ANTIGRAVITY_SESSION_ID_INVALID');
  ...
  private meta(executionId: string): AntigravityExecutionMeta {
    ...
    if (
      ...
      throw new ForgeFlowError('ANTIGRAVITY_META_INVALID');
    }
    return {
      version: 1,
      executionId,
