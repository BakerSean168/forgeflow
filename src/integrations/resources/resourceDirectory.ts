...
import type { ResourceDirectoryPort } from '../../core/orchestration/resourceSelector.js';
import type {
  MutationResult,
...
function readEnvValue(file: string, key: string): string {
  ...
  throw new ForgeFlowError('LITELLM_RESOURCE_KEY_MISSING');
}
...
export class LiteLlmResourceDirectory implements ResourceDirectoryPort {
  ...
  async refresh(): Promise<readonly ExecutionResource[]> {
    ...
    if (!response.ok) throw new ForgeFlowError('LITELLM_RESOURCE_DIRECTORY_HTTP_' + response.status);
    ...
    for (const raw of rows) {
      ...
      if (
        ...
        throw new ForgeFlowError('LITELLM_RESOURCE_DIRECTORY_ROW_INVALID');
      }
      ...
      if (
        ...
        throw new ForgeFlowError('LITELLM_RESOURCE_DIRECTORY_RESOURCE_CONFLICT');
      ...
      this.directories
        ...
        .map((resource) => {
          if (seen.has(resource.resourceId))
            throw new ForgeFlowError('RESOURCE_DIRECTORY_DUPLICATE_RESOURCE');
...
export class LiteLlmResourceStateEffect implements ResourceStateEffectPort {
  ...
  async #patch(deploymentId: string, blocked: boolean): Promise<void> {
    ...
    if (!response.ok) throw new ForgeFlowError('LITELLM_RESOURCE_STATE_HTTP_' + response.status);
  }
  ...
  async applyBinding(
    ...
    if (!binding.deploymentId) throw new ForgeFlowError('LITELLM_BINDING_DEPLOYMENT_ID_REQUIRED');
...
export class ResourceStateService implements ResourceFeedbackPort {
  ...
  #resource(id: string): ExecutionResource {
    ...
    if (!value) throw new ForgeFlowError('RESOURCE_NOT_FOUND');
    return value;
  }
