...
import type { DeliveryObservation, PlanDelivery } from '../../core/domain/delivery.js';
import { ForgeFlowError, failClosed } from '../../core/domain/errors.js';
import type { Plan } from '../../core/domain/plan.js';
...
const GOVERNANCE_CONTEXT = 'ForgeFlow / PR Governance';
...
export class GitHubCliDeliveryAdapter implements DeliveryAutomationPort {
  ...
  async advance(
    ...
    if (delivery.pullRequestNumber !== undefined) {
      ...
      if (durablePullRequest.state === 'MERGED') throw new ForgeFlowError('DELIVERY_PR_HEAD_STALE');
      ...
      if (this.git(sourceRepository, identity, ['status', '--porcelain=v1', '-z']).length !== 0)
        throw new ForgeFlowError('DELIVERY_LOCAL_REPOSITORY_DIRTY');
      ...
      if (sourceHead !== plan.currentRevision) {
        ...
        throw new ForgeFlowError('DELIVERY_LOCAL_HEAD_STALE');
      }
    }
    ...
    if (head !== plan.currentRevision) throw new ForgeFlowError('DELIVERY_LOCAL_HEAD_STALE');
    ...
    if (remoteHead && remoteHead !== plan.currentRevision) {
      ...
      if (!fastForward) throw new ForgeFlowError('DELIVERY_REMOTE_BRANCH_DIVERGED');
      this.git(
        repository,
        identity,
    ...
    if (!pullRequest) {
      const title = plan.objective.trim().slice(0, 240) || 'ForgeFlow delivery';
      const body = [
        'ForgeFlow durable delivery.',
        '',
        'Plan: `' + plan.planId + '`',
      ...
      if (!pullRequest) throw new ForgeFlowError('DELIVERY_PR_CREATE_NOT_OBSERVED');
    }
    return this.observeExactPullRequest(
      repository,
      identity,
  ...
  private observeExactPullRequest(
    ...
    if (pullRequest.headRefOid !== plan.currentRevision)
      throw new ForgeFlowError('DELIVERY_PR_HEAD_STALE');
    if (pullRequest.state === 'CLOSED') throw new ForgeFlowError('DELIVERY_PR_CLOSED_UNMERGED');
    ...
    if (mergeState === 'BEHIND') throw new ForgeFlowError('DELIVERY_TARGET_BRANCH_ADVANCED');
    if (mergeState === 'DIRTY') throw new ForgeFlowError('DELIVERY_PR_CONFLICT');
    ...
    if (merged.headRefOid !== plan.currentRevision) throw new ForgeFlowError('DELIVERY_PR_HEAD_STALE');
    if (merged.state !== 'MERGED') throw new ForgeFlowError('DELIVERY_MERGE_NOT_OBSERVED');
  ...
  private assertPullRequestIdentity(pullRequest: PullRequestView, delivery: PlanDelivery): void {
    if (pullRequest.headRefName !== undefined && pullRequest.headRefName !== delivery.branch)
      throw new ForgeFlowError('DELIVERY_PR_BRANCH_MISMATCH');
    if (pullRequest.baseRefName !== undefined && pullRequest.baseRefName !== delivery.targetBranch)
      throw new ForgeFlowError('DELIVERY_PR_BASE_MISMATCH');
  }
  ...
  private verifyMerged(
    ...
    if (!mergeSha) throw new ForgeFlowError('DELIVERY_MERGE_SHA_MISSING');
    ...
    if (!targetHead) throw new ForgeFlowError('DELIVERY_POST_MERGE_HEAD_MISMATCH');
    if (targetHead !== mergeSha) {
      ...
      if (!localAncestor) {
        ...
        if (status !== 'ahead' && status !== 'identical')
          throw new ForgeFlowError('DELIVERY_POST_MERGE_HEAD_MISMATCH');
      }
    }
  ...
  private preflightCommitLint(
    ...
    if (fs.existsSync(packageFile)) {
      try {
        ...
        throw new ForgeFlowError('DELIVERY_COMMITLINT_CONFIG_INVALID');
      }
    }
    if (!declared) return;
    const baseSha = this.remoteHead(repository, identity, delivery.remote, delivery.targetBranch);
    if (!baseSha) throw new ForgeFlowError('DELIVERY_COMMITLINT_BASE_UNAVAILABLE');
    let command: string;
    let args: string[];
    if (fs.existsSync(path.join(repository, 'pnpm-lock.yaml'))) {
      ...
      throw new ForgeFlowError('DELIVERY_COMMITLINT_RUNTIME_UNAVAILABLE');
    }
    try {
      this.command(command, args, identity, { COREPACK_ENABLE_DOWNLOAD_PROMPT: '0' }, repository);
    } catch (error) {
      if (error instanceof ForgeFlowError && error.code === 'DELIVERY_COMMAND_FAILED')
  ...
  private findPullRequest(
    ...
    if (!Array.isArray(parsed)) throw new ForgeFlowError('DELIVERY_PR_LIST_INVALID');
    const candidates = expectedHeadSha
      ...
      : parsed;
    if (candidates.length > 1) throw new ForgeFlowError('DELIVERY_PR_AMBIGUOUS');
  ...
  private ensureGovernanceStatus(
    ...
    this.gh(repository, identity, [
      ...
      '-f',
      'description=ForgeFlow exact-SHA independent review passed',
      '-f',
  ...
  private gitSucceeds(
    ...
    try {
      ...
      if (error instanceof ForgeFlowError && error.code === 'DELIVERY_COMMAND_FAILED') return false;
      throw error;
    }
  }
  ...
  private remoteHead(
    ...
    if (rows.length !== 1) throw new ForgeFlowError('DELIVERY_REMOTE_REF_AMBIGUOUS');
  ...
  private githubRepository(
    ...
    if (!match?.[1]) throw new ForgeFlowError('DELIVERY_GITHUB_REMOTE_REQUIRED');
  ...
  private command(
    ...
    if (result.error || status !== 0) {
      const detail = (stderr || result.error?.message || 'command failed')
        .replace(/\s+/g, ' ')
        .slice(0, 1_000);
      throw new ForgeFlowError('DELIVERY_COMMAND_FAILED', path.basename(command) + ': ' + detail);
    }
  ...
  private deliveryWorktree(
    ...
    if (
      ...
      throw new ForgeFlowError('DELIVERY_WORKSPACE_NOT_ALLOWED');
    const top = path.resolve(this.git(real, identity, ['rev-parse', '--show-toplevel']).trim());
    if (fs.realpathSync(top) !== real) throw new ForgeFlowError('DELIVERY_WORKSPACE_ROOT_MISMATCH');
    ...
    if (sourceCommon !== workspaceCommon)
      throw new ForgeFlowError('DELIVERY_WORKSPACE_COMMON_DIR_MISMATCH');
    const head = this.git(real, identity, ['rev-parse', '--verify', 'HEAD^{commit}']).trim();
    if (head !== expectedRevision) throw new ForgeFlowError('DELIVERY_WORKSPACE_HEAD_STALE');
    return real;
  }
  ...
  private repositoryRoot(repositoryPath: string): string {
    ...
    if (!allowed) throw new ForgeFlowError('DELIVERY_REPOSITORY_NOT_ALLOWED');
    return real;
  }
  ...
  private repositoryIdentity(repository: string): { uid: number; gid: number; home: string } {
    ...
    if (passwd.status !== 0) throw new ForgeFlowError('DELIVERY_REPOSITORY_OWNER_INVALID');
    const fields = String(passwd.stdout ?? '')
      .trim()
      .split(':');
    const home = fields[5];
    if (!home || !path.isAbsolute(home)) throw new ForgeFlowError('DELIVERY_REPOSITORY_OWNER_INVALID');
  ...
  private parseJson<T>(value: string, code: string): T {
    try {
      ...
      throw new ForgeFlowError(code, 'GitHub CLI returned invalid JSON.', error);
    }
  }
}
