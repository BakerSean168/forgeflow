...
export class PlanWorktreeRepository {
    ...
    return withTransaction(this.db, () => {
      ...
      if (existing) {
        if (
          ...
          throw new ForgeFlowError('WORKTREE_IDEMPOTENCY_CONFLICT');
  ...
  get(worktreeId: string): PlanWorktree {
    ...
    if (!value) throw new ForgeFlowError('WORKTREE_NOT_FOUND');
    return value;
  }
    ...
    return withTransaction(this.db, () => {
      ...
      if (existing.length > 0) {
        ...
        if (JSON.stringify(actual) !== JSON.stringify(normalized))
          throw new ForgeFlowError('WORKTREE_PROTECTED_REF_SNAPSHOT_CONFLICT');
        return { status: 'existing', value: existing };
      }
