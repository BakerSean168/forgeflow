...
import {
  isDeliveryComplete,
  validatePlanDeliveryConfig,
  type DeliveryObservation,
  type DeliveryStatus,
  type PlanDelivery,
  type PlanDeliveryConfig,
} from '../domain/delivery.js';
import { DuplicateKeyError, StaleStateError, ForgeFlowError, failClosed } from '../domain/errors.js';
...
import {
  validateProjectPlanLease,
  validateProjectPlanPriority,
  type ProjectPlanLease,
  type ProjectPlanQueueEntry,
  type RootPlanHandoffResult,
  type RootPlanScheduleResult,
...
import { assertCurrentSchema, withTransaction } from './database.js';
...
function scheduleSupervisorWakeForPlan(
  ...
  const supervisor = db
    .prepare('SELECT supervisor_id,observation_cursor FROM supervisors WHERE plan_id=?')
    .get(planId) as { supervisor_id: string; observation_cursor: number } | undefined;
  if (!supervisor) return;
  const row = db.prepare('SELECT COALESCE(MAX(event_order),0) AS cursor FROM events').get() as {
    cursor: number;
  };
  ...
  db.prepare(
    'INSERT OR IGNORE INTO supervisor_wakes(wake_key,supervisor_id,observation_cursor,reason,requested_at) VALUES(?,?,?,?,?)',
  ).run(wakeKey, supervisor.supervisor_id, cursor, reason, requestedAt);
}
...
function planFrom(row: PlanRow, children: string[] = []): Plan {
  return {
    ...
    status: row.status,
    activeGraphVersionId: row.active_graph_version_id ?? undefined,
    parentPlanId: row.parent_plan_id ?? undefined,
    childPlanIds: children,
...
interface DeliveryRow {
  ...
  auto_merge: number;
  merge_method: PlanDeliveryConfig['mergeMethod'];
...
interface GraphRow {
  ...
  reason: string;
  triggering_observation_cursor: number | null;
...
function graphFrom(row: GraphRow): GraphVersion {
  return {
    ...
    planId: row.plan_id,
    version: row.version,
    parentGraphVersionId: row.parent_graph_version_id ?? undefined,
    reason: row.reason,
    triggeringObservationCursor: row.triggering_observation_cursor ?? undefined,
    status: row.status,
    createdAt: row.created_at,
  };
}
...
export class PlanRepository {
  ...
  createPlan(input: CreatePlanInput): MutationResult<Plan> {
    ...
    return withTransaction(this.db, () => {
      const existing = this.db
        ...
        .get(input.idempotencyKey) as PlanRow | undefined;
      if (existing) {
        if (
          existing.project_key !== input.projectKey ||
        ...
        if (Boolean(existingDelivery) !== Boolean(requestedDelivery))
          throw new DuplicateKeyError(input.idempotencyKey);
        if (
          existingDelivery &&
          requestedDelivery &&
          !this.deliveryConfigEqual(existingDelivery, requestedDelivery)
        )
          throw new DuplicateKeyError(input.idempotencyKey);
      ...
      this.db
        .prepare(
          'INSERT INTO plans(plan_id,idempotency_key,project_key,objective,repository_path,base_revision,current_revision,status,parent_plan_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          planId,
      ...
      this.events.appendInTransaction(
        makeEvent(
          ...
          'PLAN_CREATED',
          {
            ...
            deliveryRequired: Boolean(input.delivery),
          },
          input.idempotencyKey,
        ),
      );
  ...
  createChildPlan(input: {
    ...
    return withTransaction(this.db, () => {
      ...
      let row = this.db
        .prepare('SELECT * FROM plans WHERE idempotency_key=?')
        .get(childInput.idempotencyKey) as PlanRow | undefined;
      if (row) {
        if (
          row.plan_id !== input.childPlanId ||
          row.parent_plan_id !== parent.planId ||
          row.objective !== input.objective ||
          row.repository_path !== input.repositoryPath ||
          row.base_revision !== childBaseRevision
        ) {
          throw new DuplicateKeyError(childInput.idempotencyKey);
        }
      } else {
        const conflictingId = this.db
          .prepare('SELECT plan_id FROM plans WHERE plan_id=?')
          .get(input.childPlanId);
        ...
        this.db
          ...
          .run(
            ...
            childInput.projectKey,
            childInput.objective,
            childInput.repositoryPath,
            childInput.baseRevision,
            childInput.baseRevision,
            'DRAFT',
            parent.planId,
            createdAt,
        ...
        this.events.appendInTransaction(
          makeEvent(
            ...
            {
              planId: input.childPlanId,
              projectKey: childInput.projectKey,
              objective: childInput.objective,
              repositoryPath: childInput.repositoryPath,
              baseRevision: childInput.baseRevision,
              parentPlanId: parent.planId,
            },
            childInput.idempotencyKey,
          ),
        ...
        row = this.db
          ...
          .get(input.childPlanId) as unknown as PlanRow;
      }

      const existing = this.db
        .prepare(
          'SELECT relationship_id,parent_plan_id,kind FROM plan_relationships WHERE child_plan_id=?',
        )
        .get(input.childPlanId) as
        { relationship_id: string; parent_plan_id: string; kind: string } | undefined;
      let relationshipId: string;
      if (existing) {
        if (existing.parent_plan_id !== parent.planId || existing.kind !== input.relation)
          throw new ForgeFlowError('PARENT_CHILD_PARENT_CONFLICT');
        relationshipId = existing.relationship_id;
      } else {
        if (parent.planId === input.childPlanId) throw new ForgeFlowError('PARENT_CHILD_SELF_CYCLE');
        const cycle = this.db
          .prepare(
            'WITH RECURSIVE descendants(plan_id) AS (SELECT child_plan_id FROM plan_relationships WHERE parent_plan_id=? UNION SELECT relationships.child_plan_id FROM plan_relationships relationships JOIN descendants ON relationships.parent_plan_id=descendants.plan_id) SELECT plan_id FROM descendants WHERE plan_id=? LIMIT 1',
          )
          .get(input.childPlanId, parent.planId);
        if (cycle) throw new ForgeFlowError('PARENT_CHILD_CYCLE');
        relationshipId = id('relationship');
        this.db
          .prepare(
            'INSERT INTO plan_relationships(relationship_id,parent_plan_id,child_plan_id,kind,created_at) VALUES(?,?,?,?,?)',
          )
          .run(relationshipId, parent.planId, input.childPlanId, input.relation, iso());
        this.events.appendInTransaction(
          makeEvent(relationshipId, 'RELATIONSHIP', 'PARENT_CHILD_CREATED', {
            relationshipId,
            parentPlanId: parent.planId,
      ...
      if (
        (input.relation === 'SYSTEM_REPAIR' || input.relation === 'INFRASTRUCTURE_REPAIR') &&
        (parent.status === 'READY' || parent.status === 'RUNNING')
      ) {
        ...
        this.db
          ...
          .run(updated.status, updated.updatedAt, parent.planId, parent.status);
        this.events.appendInTransaction(
          makeEvent(parent.planId, 'PLAN', 'PLAN_STATUS_CHANGED', {
            from: parent.status,
            to: updated.status,
          }),
        );
  ...
  getPlan(planId: string): Plan {
    const row = this.db.prepare('SELECT * FROM plans WHERE plan_id=?').get(planId) as
      PlanRow | undefined;
    if (!row) throw new ForgeFlowError('PLAN_NOT_FOUND', 'Plan not found: ' + planId);
    const children = this.db
      .prepare(
        'SELECT child_plan_id FROM plan_relationships WHERE parent_plan_id=? ORDER BY created_at',
      )
      .all(planId) as unknown as Array<{ child_plan_id: string }>;
    const delivery = this.getDelivery(planId);
    return {
      ...planFrom(
        row,
        children.map((child) => child.child_plan_id),
      ),
      ...(delivery ? { delivery } : {}),
    };
  }
  ...
  getDelivery(planId: string): PlanDelivery | undefined {
    const row = this.db.prepare('SELECT * FROM plan_deliveries WHERE plan_id=?').get(planId) as
  ...
  attachDelivery(planId: string, config: PlanDeliveryConfig): MutationResult<PlanDelivery> {
    ...
    return withTransaction(this.db, () => {
      ...
      if (existing) {
        if (!this.deliveryConfigEqual(existing, config))
          throw new ForgeFlowError('PLAN_DELIVERY_CONFLICT');
        return { status: 'existing', value: existing };
      }
      const createdAt = iso();
      this.insertDelivery(planId, config, createdAt);
      this.events.appendInTransaction(
        makeEvent(planId, 'DELIVERY', 'PLAN_DELIVERY_ATTACHED', {
  ...
  recordDeliveryObservation(
    ...
    return withTransaction(this.db, () => {
      ...
      if (!current) throw new ForgeFlowError('PLAN_DELIVERY_REQUIRED');
      if (DELIVERY_STAGE[observation.status] < DELIVERY_STAGE[current.status])
        throw new ForgeFlowError('DELIVERY_STATUS_REGRESSION');
      const immutable = <T>(
      ...
      const pullRequestNumber = immutable(
        current.pullRequestNumber,
        observation.pullRequestNumber,
        'DELIVERY_PR_CONFLICT',
      );
      const pullRequestUrl = immutable(
        current.pullRequestUrl,
        observation.pullRequestUrl,
        'DELIVERY_PR_CONFLICT',
      );
      const mergeSha = immutable(
        current.mergeSha,
        observation.mergeSha,
        'DELIVERY_MERGE_SHA_CONFLICT',
      );
      const updatedAt = iso();
      this.db
        .prepare(
          'UPDATE plan_deliveries SET status=?,head_sha=?,pull_request_number=?,pull_request_url=?,merge_sha=?,error_code=?,updated_at=? WHERE plan_id=?',
        )
        .run(
          observation.status,
          headSha ?? null,
      ...
      this.events.appendInTransaction(
        makeEvent(planId, 'DELIVERY', 'PLAN_DELIVERY_OBSERVED', {
  ...
  delegateDeliveryRepair(
    ...
    return withTransaction(this.db, () => {
      ...
      const current = this.getDelivery(planId);
      if (!current) throw new ForgeFlowError('PLAN_DELIVERY_REQUIRED');
      if (isDeliveryComplete(current)) throw new ForgeFlowError('DELIVERY_ALREADY_COMPLETE');
      ...
      if (!relation || relation.kind !== 'FOLLOW_UP')
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_RELATION_REQUIRED');
      if (child.parentPlanId !== planId)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_PARENT_MISMATCH');
      const expectedChildBase = current.headSha ?? parent.currentRevision;
      if (child.baseRevision !== expectedChildBase)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_BASE_MISMATCH');
      if (current.supersededByPlanId && current.supersededByPlanId !== childPlanId)
        throw new ForgeFlowError('DELIVERY_SUPERSEDED_CHILD_CONFLICT');
  ...
  supersedeDelivery(planId: string, childPlanId: string): MutationResult<PlanDelivery> {
    return withTransaction(this.db, () => {
      ...
      const current = this.getDelivery(planId);
      if (!current) throw new ForgeFlowError('PLAN_DELIVERY_REQUIRED');
      if (current.status === 'VERIFIED') throw new ForgeFlowError('DELIVERY_ALREADY_VERIFIED');
      ...
      if (child.status !== 'SUCCEEDED' || !childDelivery || !isDeliveryComplete(childDelivery))
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_NOT_VERIFIED');
      if (childDelivery.headSha !== child.currentRevision || !childDelivery.mergeSha)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_EXACT_REVISION_REQUIRED');
      if (!this.deliveryConfigEqual(current, childDelivery))
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_CONFIG_MISMATCH');
      if (
        ...
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_PR_MISMATCH');
      const updatedAt = iso();
      this.db
        .prepare(
  ...
  supersedeDeliveryFromVerifiedSibling(
    ...
    return withTransaction(this.db, () => {
      ...
      if (current.status === 'SUPERSEDED_PENDING_CHILD' || current.supersededByPlanId)
        throw new ForgeFlowError('DELIVERY_SUPERSEDED_CHILD_CONFLICT');
      if (!plan.parentPlanId) throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_PARENT_REQUIRED');
      if (plan.currentRevision !== expectedPlanRevision)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_STALE_PLAN_REVISION');
      if ((current.headSha ?? plan.currentRevision) !== expectedPlanRevision)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_DELIVERY_HEAD_MISMATCH');
      ...
      if (
        ...
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_LINEAGE_MISMATCH');
      if (sibling.currentRevision !== expectedSiblingRevision)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_STALE_VERIFIED_REVISION');
      ...
      if (!currentRelation || !siblingRelation)
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_RELATION_REQUIRED');
      ...
      if (
        ...
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_NOT_VERIFIED');
      if (
        siblingDelivery.headSha !== sibling.currentRevision ||
        siblingDelivery.headSha !== expectedSiblingRevision ||
        !siblingDelivery.mergeSha
      )
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_EXACT_REVISION_REQUIRED');
      if (!this.deliveryConfigEqual(current, siblingDelivery))
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_CONFIG_MISMATCH');
      if (
        current.pullRequestNumber !== undefined &&
        siblingDelivery.pullRequestNumber !== undefined &&
        current.pullRequestNumber !== siblingDelivery.pullRequestNumber
      )
        throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_PR_MISMATCH');
  ...
  recordDeliveryError(planId: string, errorCode: string): MutationResult<PlanDelivery> {
    ...
    return withTransaction(this.db, () => {
      ...
      const updatedAt = iso();
      this.db
        .prepare('UPDATE plan_deliveries SET error_code=?,updated_at=? WHERE plan_id=?')
        .run(errorCode, updatedAt, planId);
      this.events.appendInTransaction(
        makeEvent(planId, 'DELIVERY', 'PLAN_DELIVERY_ERROR_RECORDED', {
  ...
  private insertDelivery(planId: string, config: PlanDeliveryConfig, createdAt: string): void {
    this.db
      .prepare(
        'INSERT INTO plan_deliveries(plan_id,remote,branch,target_branch,auto_merge,merge_method,required_checks,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)',
      )
      .run(
        planId,
        config.remote,
        config.branch,
        config.targetBranch,
        config.autoMerge ? 1 : 0,
        config.mergeMethod,
        encode(config.requiredChecks),
        'PENDING',
        createdAt,
        createdAt,
      );
  ...
  getPlanByIdempotencyKey(key: string): Plan | undefined {
    const row = this.db.prepare('SELECT * FROM plans WHERE idempotency_key=?').get(key) as
      PlanRow | undefined;
  ...
  listPlans(input: { status?: PlanStatus; limit?: number } = {}): Plan[] {
    ...
    const rows = input.status
      ? (this.db
          .prepare('SELECT plan_id FROM plans WHERE status=? ORDER BY updated_at DESC LIMIT ?')
          .all(input.status, limit) as unknown as Array<{ plan_id: string }>)
      : (this.db
          .prepare('SELECT plan_id FROM plans ORDER BY updated_at DESC LIMIT ?')
          .all(limit) as unknown as Array<{ plan_id: string }>);
  ...
  reconcileCurrentRevision(
    ...
    failClosed(
      expectedRevision.trim().length > 0 && observedRevision.trim().length > 0,
      'PLAN_REVISION_REQUIRED',
    );
    failClosed(reason.trim().length > 0, 'PLAN_REVISION_REASON_REQUIRED');
    return withTransaction(this.db, () => {
      const current = this.getPlan(planId);
      if (current.currentRevision === observedRevision)
        return { status: 'existing', value: current };
      if (current.currentRevision !== expectedRevision)
        return { status: 'rejected', value: current, reason: 'STALE_REVISION' };
      if (
        current.status === 'SUCCEEDED' ||
        current.status === 'FAILED' ||
        current.status === 'CANCELLED'
      )
        throw new ForgeFlowError('PLAN_REVISION_TERMINAL');
      const updatedAt = iso();
      const result = this.db
        .prepare(
          'UPDATE plans SET current_revision=?,updated_at=? WHERE plan_id=? AND current_revision=?',
        )
        .run(observedRevision, updatedAt, planId, expectedRevision);
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getPlan(planId), reason: 'STALE_REVISION' };
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PLAN_REVISION_RECONCILED', {
  ...
  advanceAcceptedRevision(
    ...
    failClosed(
      expectedRevision.trim().length > 0 && acceptedRevision.trim().length > 0,
      'PLAN_REVISION_REQUIRED',
    );
    failClosed(reason.trim().length > 0, 'PLAN_REVISION_REASON_REQUIRED');
    return withTransaction(this.db, () => {
      const current = this.getPlan(planId);
      if (current.currentRevision === acceptedRevision)
        return { status: 'existing', value: current };
      if (current.currentRevision !== expectedRevision)
        return { status: 'rejected', value: current, reason: 'STALE_REVISION' };
      if (current.status !== 'RUNNING') throw new ForgeFlowError('PLAN_NOT_RUNNING');
      const updatedAt = iso();
      const result = this.db
        .prepare(
          'UPDATE plans SET current_revision=?,updated_at=? WHERE plan_id=? AND current_revision=? AND status=?',
        )
        .run(acceptedRevision, updatedAt, planId, expectedRevision, 'RUNNING');
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getPlan(planId), reason: 'STALE_REVISION' };
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PLAN_ACCEPTED_REVISION_ADVANCED', {
          from: expectedRevision,
          to: acceptedRevision,
          reason,
        }),
  ...
  updateStatus(planId: string, next: PlanStatus): MutationResult<Plan> {
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=?')
        .run(next, updated.updatedAt, planId);
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PLAN_STATUS_CHANGED', { from: current.status, to: next }),
      );
  ...
  compareAndSetStatus(
    ...
    return withTransaction(this.db, () => {
      ...
      if (current.status !== expected)
        return { status: 'rejected', value: current, reason: 'STALE_STATUS' };
      if (expected === next) return { status: 'existing', value: current };
      const updated = transitionPlan(current, next, iso());
      const result = this.db
        .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=? AND status=?')
        .run(next, updated.updatedAt, planId, expected);
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getPlan(planId), reason: 'STALE_STATUS' };
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PLAN_STATUS_CHANGED', { from: expected, to: next }),
      );
      return { status: 'updated', value: this.getPlan(planId) };
    });
  }

  createGraphVersion(input: {
    planId: string;
    reason: string;
    parentGraphVersionId?: string;
    triggeringObservationCursor?: number;
    version?: number;
  }): MutationResult<GraphVersion> {
    failClosed(input.reason.trim().length > 0, 'GRAPH_REASON_REQUIRED');
    return withTransaction(this.db, () => {
      this.getPlan(input.planId);
      const previous = this.db
        .prepare('SELECT graph_version_id FROM graph_versions WHERE plan_id=? AND status=?')
        .get(input.planId, 'ACTIVE') as { graph_version_id: string } | undefined;
      const latest = this.db
        .prepare('SELECT COALESCE(MAX(version),0) AS version FROM graph_versions WHERE plan_id=?')
        .get(input.planId) as { version: number };
      const version = input.version ?? latest.version + 1;
      if (version !== latest.version + 1)
        throw new StaleStateError('Graph version must be append-only');
      const graph: GraphVersion = {
        graphVersionId: id('graph'),
        planId: input.planId,
        version,
        parentGraphVersionId: input.parentGraphVersionId,
        reason: input.reason,
        triggeringObservationCursor: input.triggeringObservationCursor,
        status: 'ACTIVE',
        createdAt: iso(),
      };
      this.db
        .prepare('UPDATE graph_versions SET status=? WHERE plan_id=? AND status=?')
        .run('SUPERSEDED', input.planId, 'ACTIVE');
      this.db
        .prepare(
          'INSERT INTO graph_versions(graph_version_id,plan_id,version,parent_graph_version_id,reason,triggering_observation_cursor,status,created_at) VALUES(?,?,?,?,?,?,?,?)',
        )
        .run(
          graph.graphVersionId,
          graph.planId,
          graph.version,
          graph.parentGraphVersionId ?? null,
          graph.reason,
          graph.triggeringObservationCursor ?? null,
          graph.status,
          graph.createdAt,
        );
      this.db
        .prepare('UPDATE plans SET active_graph_version_id=?,updated_at=? WHERE plan_id=?')
        .run(graph.graphVersionId, graph.createdAt, input.planId);
      if (previous)
        this.events.appendInTransaction(
          makeEvent(previous.graph_version_id, 'PLAN', 'GRAPH_VERSION_SUPERSEDED', {
            planId: input.planId,
            graphVersionId: previous.graph_version_id,
          }),
        );
      this.events.appendInTransaction(
        makeEvent(graph.graphVersionId, 'PLAN', 'GRAPH_VERSION_CREATED', {
          planId: input.planId,
          version,
          reason: input.reason,
        }),
  ...
  supersedeGraphVersion(planId: string, graphVersionId: string): MutationResult<GraphVersion> {
    return withTransaction(this.db, () => {
      ...
      if (graph.planId !== planId) throw new ForgeFlowError('GRAPH_PLAN_MISMATCH');
      if (graph.status === 'SUPERSEDED') return { status: 'existing', value: graph };
      if (graph.status !== 'ACTIVE')
        return { status: 'rejected', value: graph, reason: 'GRAPH_NOT_ACTIVE' };
      this.db
        .prepare('UPDATE graph_versions SET status=? WHERE graph_version_id=? AND status=?')
        .run('SUPERSEDED', graphVersionId, 'ACTIVE');
      this.events.appendInTransaction(
        makeEvent(graphVersionId, 'PLAN', 'GRAPH_VERSION_SUPERSEDED', { planId, graphVersionId }),
      );
  ...
  getActiveGraphVersion(planId: string): GraphVersion | undefined {
    const row = this.db
      .prepare('SELECT * FROM graph_versions WHERE plan_id=? AND status=?')
      .get(planId, 'ACTIVE') as GraphRow | undefined;
    return row ? graphFrom(row) : undefined;
  }

  getGraphVersion(graphVersionId: string): GraphVersion {
    const row = this.db
      .prepare('SELECT * FROM graph_versions WHERE graph_version_id=?')
      .get(graphVersionId) as GraphRow | undefined;
    if (!row) throw new ForgeFlowError('GRAPH_VERSION_NOT_FOUND');
    return graphFrom(row);
  }

  appendGraphWorkItem(input: {
    ...
    return withTransaction(this.db, () => {
      ...
      if (graph.status !== 'ACTIVE') throw new ForgeFlowError('GRAPH_VERSION_NOT_ACTIVE');
      ...
      validateGraphItems([
        ...existing.map((item) => ({ itemKey: item.itemKey, dependencies: item.dependencies })),
      ...
      this.events.appendInTransaction(
        makeEvent(item.workItemId, 'WORK_ITEM', 'WORK_ITEM_APPENDED', {
          planId: item.planId,
  ...
  listWorkItems(planId: string, graphVersionId?: string): WorkItem[] {
    ...
    const rows = this.db
      .prepare(
        'SELECT * FROM work_items WHERE plan_id=? AND graph_version_id=? ORDER BY created_at,item_key',
      )
      .all(planId, versionId) as unknown as WorkRow[];
    return rows.map(workFrom);
  }

  getWorkItem(workItemId: string): WorkItem {
    const row = this.db.prepare('SELECT * FROM work_items WHERE work_item_id=?').get(workItemId) as
      WorkRow | undefined;
    if (!row) throw new ForgeFlowError('WORK_ITEM_NOT_FOUND');
    return workFrom(row);
  }
  ...
  updateWorkItemStatus(workItemId: string, next: WorkItemStatus): MutationResult<WorkItem> {
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare('UPDATE work_items SET status=?,updated_at=? WHERE work_item_id=?')
        .run(next, updated.updatedAt, workItemId);
      this.events.appendInTransaction(
        makeEvent(workItemId, 'WORK_ITEM', 'WORK_ITEM_STATUS_CHANGED', {
  ...
  recoverFailedPlanWorkItem(
    planId: string,
    workItemId: string,
  ): { plan: Plan; workItem: WorkItem } {
    return withTransaction(this.db, () => {
      const plan = this.getPlan(planId);
      const item = this.getWorkItem(workItemId);
      failClosed(item.planId === planId, 'PLAN_RECOVERY_WORK_ITEM_MISMATCH');
      if (plan.status === 'RUNNING' && item.status === 'RUNNING') return { plan, workItem: item };
      ...
      const itemResult = this.db
        ...
        .run('RUNNING', runningItem.updatedAt, workItemId, item.status);
  ...
  acceptWorkItemRevision(
    ...
    return withTransaction(this.db, () => {
      ...
      if (current.status === 'SUCCEEDED') {
        if (current.exactAcceptedRevision === revision)
          return { status: 'existing', value: current };
        throw new ForgeFlowError('WORK_ITEM_ACCEPTED_REVISION_IMMUTABLE');
      }
      if (current.status !== 'RUNNING') throw new ForgeFlowError('WORK_ITEM_NOT_RUNNING');
      const plan = this.getPlan(current.planId);
      if (plan.currentRevision !== expectedPlanRevision)
        throw new ForgeFlowError('WORK_ITEM_PLAN_REVISION_NOT_CURRENT');
      const updated = transitionWorkItem(current, 'SUCCEEDED', iso());
      const result = this.db
        .prepare(
          'UPDATE work_items SET status=?,exact_accepted_revision=?,updated_at=? WHERE work_item_id=? AND status=? AND exact_accepted_revision IS NULL',
        )
        .run('SUCCEEDED', revision, updated.updatedAt, workItemId, 'RUNNING');
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getWorkItem(workItemId), reason: 'STALE_STATUS' };
      this.events.appendInTransaction(
  ...
  rejectCompletedWorkRemoval(oldVersionId: string, proposedKeys: readonly string[]): void {
    const rows = this.db
      .prepare('SELECT item_key FROM work_items WHERE graph_version_id=? AND status=?')
      .all(oldVersionId, 'SUCCEEDED') as unknown as Array<{ item_key: string }>;
    const keys = new Set(proposedKeys);
    for (const row of rows)
      if (!keys.has(row.item_key)) throw new ForgeFlowError('COMPLETED_WORK_REMOVAL_REJECTED');
  }
}
...
export class ExecutionRepository {
  ...
  create(input: {
    ...
    failClosed(
      input.idempotencyKey.trim().length > 0 && input.objective.trim().length > 0,
    ...
    return withTransaction(this.db, () => {
      const existing = this.db
        ...
        .get(input.idempotencyKey) as ExecutionRow | undefined;
      if (existing) {
        const same =
          existing.execution_id === requestedId &&
      ...
      const idConflict = this.db
        .prepare('SELECT idempotency_key FROM executions WHERE execution_id=?')
        .get(requestedId) as { idempotency_key: string } | undefined;
      if (idConflict) throw new DuplicateKeyError(requestedId);
      const plan = this.db
        .prepare('SELECT plan_id FROM plans WHERE plan_id=?')
        .get(input.identity.planId) as { plan_id: string } | undefined;
      if (!plan) throw new ForgeFlowError('PLAN_NOT_FOUND');
      if (input.identity.workItemId) {
        const workItem = this.db
          .prepare('SELECT plan_id FROM work_items WHERE work_item_id=?')
          .get(input.identity.workItemId) as { plan_id: string } | undefined;
        if (!workItem || workItem.plan_id !== input.identity.planId)
          throw new ForgeFlowError('EXECUTION_WORK_ITEM_MISMATCH');
      }
      if (input.identity.phase === 'IMPLEMENT' && input.identity.parentExecutionId) {
        const parent = this.get(input.identity.parentExecutionId);
        if (
          parent.identity.planId !== input.identity.planId ||
          parent.identity.workItemId !== input.identity.workItemId ||
          parent.identity.phase !== 'IMPLEMENT'
        ) {
          throw new ForgeFlowError('EXECUTION_PARENT_INVALID');
        }
        if (
          ...
          throw new ForgeFlowError('EXECUTION_PARENT_INVALID');
        if (parent.identity.sourceRevision !== input.identity.sourceRevision)
          throw new ForgeFlowError('EXECUTION_PARENT_REVISION_MISMATCH');
      }
      if (input.identity.phase === 'REVIEW' || input.identity.phase === 'IMPLEMENT_FIX') {
        if (!input.identity.parentExecutionId) throw new ForgeFlowError('EXECUTION_PARENT_REQUIRED');
        const parent = this.get(input.identity.parentExecutionId);
        if (
          parent.identity.planId !== input.identity.planId ||
          parent.identity.workItemId !== input.identity.workItemId
        ...
        if (
          ...
          parent.resultRevision !== input.identity.sourceRevision
        ) {
          throw new ForgeFlowError('EXECUTION_PARENT_REVISION_MISMATCH');
        }
        if (input.identity.phase === 'IMPLEMENT_FIX' && parent.identity.phase === 'REVIEW')
          throw new ForgeFlowError('EXECUTION_PARENT_INVALID');
      }
      const createdAt = iso();
      this.db
        .prepare(
          'INSERT INTO executions(execution_id,idempotency_key,plan_id,work_item_id,phase,parent_execution_id,attempt,route,source_revision,objective,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          input.executionId ?? input.identity.executionId,
          input.idempotencyKey,
          input.identity.planId,
          input.identity.workItemId ?? null,
          input.identity.phase,
          input.identity.parentExecutionId ?? null,
          input.identity.attempt,
          input.identity.route,
      ...
      this.events.appendInTransaction(
        makeEvent(input.identity.executionId, 'EXECUTION', 'EXECUTION_CREATED', {
          planId: input.identity.planId,
          workItemId: input.identity.workItemId ?? null,
          phase: input.identity.phase,
          parentExecutionId: input.identity.parentExecutionId ?? null,
  ...
  get(executionId: string): Execution {
    ...
    if (!row) throw new ForgeFlowError('EXECUTION_NOT_FOUND');
    return executionFrom(row);
  }

  findByIdempotencyKey(key: string): Execution | undefined {
    const row = this.db.prepare('SELECT * FROM executions WHERE idempotency_key=?').get(key) as
  ...
  list(input: { planId?: string; status?: ExecutionStatus; limit?: number } = {}): Execution[] {
    ...
    const rows = this.db
      .prepare(
        'SELECT * FROM executions' + where + ' ORDER BY updated_at DESC,execution_id LIMIT ?',
      )
  ...
  listByPlan(planId: string): Execution[] {
    const rows = this.db
      .prepare('SELECT * FROM executions WHERE plan_id=? ORDER BY created_at,execution_id')
      .all(planId) as unknown as ExecutionRow[];
    return rows.map(executionFrom);
  }

  listByWorkItem(workItemId: string): Execution[] {
    const rows = this.db
      .prepare('SELECT * FROM executions WHERE work_item_id=? ORDER BY attempt,created_at')
      .all(workItemId) as unknown as ExecutionRow[];
    return rows.map(executionFrom);
  }
  ...
  listByStatuses(statuses: readonly ExecutionStatus[], limit = 100): Execution[] {
    ...
    const rows = this.db
      ...
      .all(...statuses, safeLimit) as unknown as ExecutionRow[];
    return rows.map(executionFrom);
  }
  ...
  claimLease(
    ...
    ttlMs: number,
    at = Date.now(),
  ): MutationResult<Lease> {
    this.get(executionId);
    return new LeaseRepository(this.db, this.events).claim(
      'EXECUTION',
      executionId,
      ownerId,
      ttlMs,
  ...
  renewLease(
    ...
    return new LeaseRepository(this.db, this.events).renew(
      'EXECUTION',
      executionId,
      ownerId,
      leaseToken,
  ...
  releaseLease(executionId: string, ownerId: string, leaseToken: string): MutationResult<void> {
    return new LeaseRepository(this.db, this.events).release(
      'EXECUTION',
      executionId,
      ownerId,
      leaseToken,
  ...
  updateStatus(executionId: string, next: ExecutionStatus): MutationResult<Execution> {
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare('UPDATE executions SET status=?,updated_at=? WHERE execution_id=?')
        .run(next, updated.updatedAt, executionId);
      this.events.appendInTransaction(
        makeEvent(executionId, 'EXECUTION', 'EXECUTION_STATUS_CHANGED', {
          planId: current.identity.planId,
          from: current.status,
          to: next,
        }),
      );
  ...
  compareAndSetStatus(
    ...
    return withTransaction(this.db, () => {
      ...
      const result = this.db
        .prepare('UPDATE executions SET status=?,updated_at=? WHERE execution_id=? AND status=?')
        .run(next, iso(), executionId, expected);
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.get(executionId), reason: 'STALE_STATUS' };
      this.events.appendInTransaction(
        makeEvent(executionId, 'EXECUTION', 'EXECUTION_STATUS_CHANGED', {
          planId: current.identity.planId,
          from: expected,
  ...
  recordResult(
    ...
    return withTransaction(this.db, () => {
      ...
      if (input.status === 'SUCCEEDED')
        failClosed(Boolean(input.resultRevision), 'EXECUTION_RESULT_REVISION_REQUIRED');
      if (current.status === input.status) {
        ...
        throw new ForgeFlowError('EXECUTION_RESULT_IMMUTABLE');
      }
      const updated = transitionExecution(current, input.status, iso());
      this.db
        .prepare(
          'UPDATE executions SET status=?,result_revision=?,result_summary=?,error_code=?,retryable=?,updated_at=? WHERE execution_id=?',
        )
        .run(
          input.status,
      ...
      this.events.appendInTransaction(
        makeEvent(executionId, 'EXECUTION', 'EXECUTION_RESULT_RECORDED', {
          status: input.status,
...
function reviewFrom(row: ReviewRow): Review {
  return {
    ...
    planId: row.plan_id,
    workItemId: row.work_item_id ?? undefined,
    implementationExecutionId: row.implementation_execution_id,
    reviewerExecutionId: row.reviewer_execution_id ?? undefined,
    sourceRevision: row.source_revision,
    reviewedSha: row.reviewed_sha,
    status: row.status,
    verdict: row.verdict ?? undefined,
    findings: row.findings ? decode<string[]>(row.findings) : undefined,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}
...
function reviewerBinding(
  db: DatabaseSync,
  executionId: string,
): ReviewExecutionBindingRow | undefined {
  return db
    .prepare(
      `SELECT executions.plan_id,executions.work_item_id,executions.source_revision,executions.status,executions.result_revision,
    ...
    .get(executionId) as ReviewExecutionBindingRow | undefined;
}

function assertReviewerBinding(
  db: DatabaseSync,
  review: Pick<Review, 'planId' | 'workItemId' | 'implementationExecutionId' | 'sourceRevision'>,
  reviewerExecutionId: string,
): ReviewExecutionBindingRow {
  if (reviewerExecutionId === review.implementationExecutionId)
    throw new ForgeFlowError('REVIEWER_NOT_INDEPENDENT');
  ...
  if (
    ...
    throw new ForgeFlowError('REVIEWER_EXECUTION_INVALID');
  }
  if (reviewer.phase !== 'REVIEW' || !reviewer.provider_session_id)
    throw new ForgeFlowError('REVIEWER_SESSION_INVALID');
  const implementation = reviewerBinding(db, review.implementationExecutionId);
  if (
    implementation?.provider_session_id &&
    implementation.provider_session_id === reviewer.provider_session_id
  ) {
    throw new ForgeFlowError('REVIEWER_NOT_INDEPENDENT');
  }
  return reviewer;
...
function assertReviewCompletion(db: DatabaseSync, review: Review): void {
  if (!review.reviewerExecutionId) throw new ForgeFlowError('REVIEWER_EXECUTION_REQUIRED');
...
export class ReviewRepository {
  ...
  create(input: {
    ...
    return withTransaction(this.db, () => {
      const existing = this.db
        .prepare('SELECT * FROM reviews WHERE idempotency_key=?')
      ...
      if (existing) {
        const same =
          ...
          (input.reviewerExecutionId === undefined ||
            existing.reviewer_execution_id === input.reviewerExecutionId);
        if (!same) throw new DuplicateKeyError(input.idempotencyKey);
        return { status: 'existing', value: reviewFrom(existing) };
      }
      const execution = this.db
        .prepare(
          'SELECT plan_id,work_item_id,source_revision,status,result_revision FROM executions WHERE execution_id=?',
        )
        .get(input.implementationExecutionId) as
        | {
            plan_id: string;
            work_item_id: string | null;
            source_revision: string | null;
        ...
        | undefined;
      if (!execution) throw new ForgeFlowError('IMPLEMENTATION_EXECUTION_NOT_FOUND');
      if (execution.plan_id !== input.planId) throw new ForgeFlowError('REVIEW_PLAN_MISMATCH');
      if (execution.status !== 'SUCCEEDED' || !execution.result_revision)
        throw new ForgeFlowError('IMPLEMENTATION_EXECUTION_NOT_SUCCEEDED');
      if (execution.work_item_id !== (input.workItemId ?? null))
        throw new ForgeFlowError('REVIEW_WORK_ITEM_MISMATCH');
      if (input.workItemId) {
        const workItem = this.db
          .prepare('SELECT plan_id FROM work_items WHERE work_item_id=?')
          .get(input.workItemId) as { plan_id: string } | undefined;
        if (!workItem || workItem.plan_id !== input.planId)
          throw new ForgeFlowError('REVIEW_WORK_ITEM_MISMATCH');
      }
      if (execution.result_revision !== input.sourceRevision)
        throw new ForgeFlowError('REVIEW_SOURCE_NOT_IMPLEMENTATION_RESULT');
      if (input.reviewerExecutionId) {
        assertReviewerBinding(
          this.db,
          {
            planId: input.planId,
      ...
      this.db
        .prepare(
          'INSERT INTO reviews(review_id,idempotency_key,plan_id,work_item_id,implementation_execution_id,reviewer_execution_id,source_revision,reviewed_sha,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          reviewId,
          input.idempotencyKey,
          ...
          input.reviewerExecutionId ?? null,
      ...
      this.events.appendInTransaction(
        makeEvent(reviewId, 'REVIEW', 'REVIEW_CREATED', {
          planId: input.planId,
          implementationExecutionId: input.implementationExecutionId,
          sourceRevision: input.sourceRevision,
        }),
  ...
  getById(reviewId: string): Review {
    const row = this.db.prepare('SELECT * FROM reviews WHERE review_id=?').get(reviewId) as
      ReviewRow | undefined;
    if (!row) throw new ForgeFlowError('REVIEW_NOT_FOUND');
    return reviewFrom(row);
  }

  listByPlan(planId: string): Review[] {
    const rows = this.db
      .prepare('SELECT * FROM reviews WHERE plan_id=? ORDER BY created_at')
      .all(planId) as unknown as ReviewRow[];
    return rows.map(reviewFrom);
  }

  listByWorkItem(workItemId: string): Review[] {
    const rows = this.db
      .prepare('SELECT * FROM reviews WHERE work_item_id=? ORDER BY created_at')
      .all(workItemId) as unknown as ReviewRow[];
    return rows.map(reviewFrom);
  }
  ...
  findByImplementationExecution(implementationExecutionId: string): Review | undefined {
    const row = this.db
      .prepare(
        'SELECT * FROM reviews WHERE implementation_execution_id=? ORDER BY created_at DESC LIMIT 1',
      )
      .get(implementationExecutionId) as ReviewRow | undefined;
  ...
  findByReviewerExecution(reviewerExecutionId: string): Review | undefined {
    const row = this.db
      .prepare(
        'SELECT * FROM reviews WHERE reviewer_execution_id=? ORDER BY created_at DESC LIMIT 1',
      )
      .get(reviewerExecutionId) as ReviewRow | undefined;
    return row ? reviewFrom(row) : undefined;
  }
  ...
  attachReviewerExecution(reviewId: string, reviewerExecutionId: string): MutationResult<Review> {
    ...
    return withTransaction(this.db, () => {
      ...
      if (current.reviewerExecutionId === reviewerExecutionId)
        return { status: 'existing', value: current };
      if (current.reviewerExecutionId) throw new ForgeFlowError('REVIEWER_EXECUTION_IMMUTABLE');
      assertReviewerBinding(this.db, current, reviewerExecutionId);
      const updatedAt = iso();
      const result = this.db
        .prepare(
          'UPDATE reviews SET reviewer_execution_id=?,updated_at=? WHERE review_id=? AND reviewer_execution_id IS NULL',
        )
        .run(reviewerExecutionId, updatedAt, reviewId);
      if (Number(result.changes) !== 1)
        return {
          status: 'rejected',
          value: this.getById(reviewId),
          reason: 'STALE_REVIEWER_EXECUTION',
        };
      this.events.appendInTransaction(
        makeEvent(reviewId, 'REVIEW', 'REVIEWER_EXECUTION_ATTACHED', {
          reviewerExecutionId,
          implementationExecutionId: current.implementationExecutionId,
        }),
      );
  ...
  updateStatus(reviewId: string, next: ReviewStatus): MutationResult<Review> {
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare('UPDATE reviews SET status=?,updated_at=? WHERE review_id=?')
        .run(next, updated.updatedAt, reviewId);
      this.events.appendInTransaction(
        makeEvent(reviewId, 'REVIEW', 'REVIEW_STATUS_CHANGED', {
          planId: current.planId,
          from: current.status,
          to: next,
        }),
  ...
  recordVerdict(
    ...
    return withTransaction(this.db, () => {
      ...
      const next: ReviewStatus =
        verdict === 'PASS' ? 'PASSED' : verdict === 'INVALID' ? 'STALE' : 'FAILED';
      const sameFindings = JSON.stringify(current.findings ?? []) === JSON.stringify(findings);
      if (current.status === next) {
        if (current.verdict === verdict && sameFindings)
          return { status: 'existing', value: current };
        throw new ForgeFlowError('REVIEW_VERDICT_IMMUTABLE');
      }
      transitionReview(current, next, iso());
      const updatedAt = iso();
      this.db
        .prepare('UPDATE reviews SET status=?,verdict=?,findings=?,updated_at=? WHERE review_id=?')
        .run(next, verdict, encode(findings), updatedAt, reviewId);
      this.events.appendInTransaction(
        makeEvent(reviewId, 'REVIEW', 'REVIEW_VERDICT_RECORDED', {
          verdict,
          reviewedSha: current.reviewedSha,
        }),
      );
      return { status: 'updated', value: this.getById(reviewId) };
    });
...
export class SupervisorRepository {
  ...
  create(input: {
    ...
    budgetId?: string;
    nextWakeAt?: string;
  }): MutationResult<Supervisor> {
    return withTransaction(this.db, () => {
      const existing = this.db
        .prepare('SELECT * FROM supervisors WHERE plan_id=?')
        .get(input.planId) as SupervisorRow | undefined;
      if (existing) return { status: 'existing', value: this.getById(existing.supervisor_id) };
      const createdAt = iso();
      const supervisor: Supervisor = {
        supervisorId: input.supervisorId ?? id('supervisor'),
        planId: input.planId,
        status: 'CREATED',
        observationCursor: 0,
        projectionDigest: '',
        ...
        nextWakeAt: input.nextWakeAt ?? createdAt,
        createdAt,
        updatedAt: createdAt,
      };
      ...
      this.db
        .prepare(
          'INSERT INTO supervisors(supervisor_id,plan_id,status,observation_cursor,projection_digest,policy_id,budget_id,next_wake_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          ...
          supervisor.policyId,
          supervisor.budgetId,
          supervisor.nextWakeAt,
          createdAt,
          createdAt,
        );
      this.events.appendInTransaction(
        makeEvent(supervisor.supervisorId, 'SUPERVISOR', 'SUPERVISOR_CREATED', {
          planId: input.planId,
        }),
      );
  ...
  getById(supervisorId: string): Supervisor {
    const row = this.db
      .prepare('SELECT * FROM supervisors WHERE supervisor_id=?')
      .get(supervisorId) as SupervisorRow | undefined;
    if (!row) throw new ForgeFlowError('SUPERVISOR_NOT_FOUND');
    const lease = this.db
      .prepare('SELECT * FROM leases WHERE aggregate_type=? AND aggregate_id=?')
      .get('SUPERVISOR', supervisorId) as LeaseRow | undefined;
    return supervisorFrom(row, lease ? leaseFrom(lease) : undefined);
  }

  getByPlanId(planId: string): Supervisor | undefined {
    const row = this.db.prepare('SELECT * FROM supervisors WHERE plan_id=?').get(planId) as
  ...
  deferWake(supervisorId: string, nextWakeAt: string): MutationResult<Supervisor> {
    ...
    const result = this.db
      .prepare('UPDATE supervisors SET next_wake_at = ?, updated_at = ? WHERE supervisor_id = ?')
      .run(nextWakeAt, updatedAt, supervisorId);
    if (result.changes !== 1) throw new ForgeFlowError('SUPERVISOR_NOT_FOUND');
  ...
  updateStatus(supervisorId: string, next: SupervisorStatus): MutationResult<Supervisor> {
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare('UPDATE supervisors SET status=?,updated_at=? WHERE supervisor_id=?')
        .run(next, updated.updatedAt, supervisorId);
      this.events.appendInTransaction(
        makeEvent(supervisorId, 'SUPERVISOR', 'SUPERVISOR_STATUS_CHANGED', {
          planId: current.planId,
          from: current.status,
          to: next,
        }),
  ...
  compareAndSetStatus(
    ...
    return withTransaction(this.db, () => {
      ...
      const updatedAt = iso();
      const result = this.db
        .prepare('UPDATE supervisors SET status=?,updated_at=? WHERE supervisor_id=? AND status=?')
        .run(next, updatedAt, supervisorId, expected);
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getById(supervisorId), reason: 'STALE_STATUS' };
      this.events.appendInTransaction(
        makeEvent(supervisorId, 'SUPERVISOR', 'SUPERVISOR_STATUS_CHANGED', {
          planId: current.planId,
          from: expected,
  ...
  attachConversation(supervisorId: string, conversationId: string): MutationResult<Supervisor> {
    return withTransaction(this.db, () => {
      ...
      if (current.status === 'COMPLETED' || current.status === 'CANCELLED')
        throw new ForgeFlowError('SUPERVISOR_TERMINAL');
      if (current.conversationId && current.conversationId !== conversationId)
        throw new ForgeFlowError('SUPERVISOR_CONVERSATION_IMMUTABLE');
      const updatedAt = iso();
      this.db
        .prepare('UPDATE supervisors SET conversation_id=?,updated_at=? WHERE supervisor_id=?')
        .run(conversationId, updatedAt, supervisorId);
      this.events.appendInTransaction(
        makeEvent(supervisorId, 'SUPERVISOR', 'SUPERVISOR_CONVERSATION_ATTACHED', {
          conversationId,
        }),
      );
      return { status: 'updated', value: this.getById(supervisorId) };
  ...
  replaceConversation(
    ...
    return withTransaction(this.db, () => {
      const current = this.getById(supervisorId);
      if (current.conversationId === conversationId) return { status: 'existing', value: current };
      if (current.conversationId !== expectedConversationId)
        return { status: 'rejected', value: current, reason: 'STALE_CONVERSATION' };
      if (current.status === 'COMPLETED' || current.status === 'CANCELLED')
        throw new ForgeFlowError('SUPERVISOR_TERMINAL');
      const updatedAt = iso();
      const result =
        expectedConversationId === undefined
          ? this.db
              .prepare(
                'UPDATE supervisors SET conversation_id=?,updated_at=? WHERE supervisor_id=? AND conversation_id IS NULL',
              )
              .run(conversationId, updatedAt, supervisorId)
          : this.db
              .prepare(
                'UPDATE supervisors SET conversation_id=?,updated_at=? WHERE supervisor_id=? AND conversation_id=?',
              )
              .run(conversationId, updatedAt, supervisorId, expectedConversationId);
      if (Number(result.changes) !== 1)
        return {
          status: 'rejected',
          value: this.getById(supervisorId),
          reason: 'STALE_CONVERSATION',
        };
      this.events.appendInTransaction(
        makeEvent(supervisorId, 'SUPERVISOR', 'SUPERVISOR_CONVERSATION_REPLACED', {
          from: expectedConversationId ?? null,
          to: conversationId,
          reason,
        }),
      );
  ...
  advanceObservation(
    ...
    return withTransaction(this.db, () => {
      ...
      if (cursor < current.observationCursor)
        return { status: 'rejected', value: current, reason: 'STALE_CURSOR' };
      if (cursor === current.observationCursor) {
        if (digest === current.projectionDigest) return { status: 'existing', value: current };
        throw new ForgeFlowError('PROJECTION_DIGEST_CONFLICT');
      }
      const updatedAt = iso();
      this.db
        .prepare(
          'UPDATE supervisors SET observation_cursor=?,projection_digest=?,next_wake_at=?,last_decision_at=?,updated_at=? WHERE supervisor_id=? AND observation_cursor<=?',
        )
        .run(cursor, digest, nextWakeAt, updatedAt, updatedAt, supervisorId, cursor);
      this.events.appendInTransaction(
        makeEvent(supervisorId, 'SUPERVISOR', 'SUPERVISOR_OBSERVATION_ADVANCED', {
          cursor,
  ...
  listStale(cutoff = new Date(Date.now() - 300000).toISOString()): Supervisor[] {
    const rows = this.db
      .prepare(
        "SELECT * FROM supervisors WHERE status NOT IN ('COMPLETED','CANCELLED') AND updated_at<? ORDER BY updated_at",
      )
      .all(cutoff) as unknown as SupervisorRow[];
    return rows.map((row) => this.getById(row.supervisor_id));
  }
}
...
export class LeaseRepository {
  ...
  claim(
    ...
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare(
          'INSERT INTO leases(aggregate_type,aggregate_id,owner_id,lease_token,claimed_at,expires_at) VALUES(?,?,?,?,?,?) ON CONFLICT(aggregate_type,aggregate_id) DO UPDATE SET owner_id=excluded.owner_id,lease_token=excluded.lease_token,claimed_at=excluded.claimed_at,expires_at=excluded.expires_at',
        )
        .run(
          lease.aggregateType,
          lease.aggregateId,
      ...
      this.events.appendInTransaction(
        makeEvent(aggregateId, aggregateType, aggregateType + '_LEASE_CLAIMED', {
          ownerId,
  ...
  renew(
    ...
    return withTransaction(this.db, () => {
      ...
      if (
        ...
        return {
          status: 'rejected',
          value: current ? leaseFrom(current) : undefined,
          reason: 'LEASE_STALE',
        };
      const expiresAt = at + ttlMs;
      this.db
        .prepare(
          'UPDATE leases SET expires_at=? WHERE aggregate_type=? AND aggregate_id=? AND owner_id=? AND lease_token=?',
        )
        .run(expiresAt, aggregateType, aggregateId, ownerId, leaseToken);
      this.events.appendInTransaction(
        makeEvent(aggregateId, aggregateType, aggregateType + '_LEASE_RENEWED', {
          ownerId,
          expiresAt,
        }),
      );
  ...
  release(
    ...
    return withTransaction(this.db, () => {
      const result = this.db
        .prepare(
          'DELETE FROM leases WHERE aggregate_type=? AND aggregate_id=? AND owner_id=? AND lease_token=?',
        )
        .run(aggregateType, aggregateId, ownerId, leaseToken);
      if (Number(result.changes) !== 1) return { status: 'rejected', reason: 'LEASE_STALE' };
      this.events.appendInTransaction(
        makeEvent(aggregateId, aggregateType, aggregateType + '_LEASE_RELEASED', { ownerId }),
      );
...
export class DecisionRepository {
  ...
  append(decision: SupervisorDecision): MutationResult<SupervisorDecision> {
    ...
    failClosed(
      decision.action.planId === decision.planId &&
        decision.action.supervisorId === decision.supervisorId,
      'DECISION_ACTION_CONTEXT_MISMATCH',
    );
    failClosed(
      decision.action.observationCursor === decision.observationCursor &&
        decision.action.projectionDigest === decision.projectionDigest,
      'DECISION_ACTION_OBSERVATION_MISMATCH',
    );
    failClosed(
      decision.action.idempotencyKey === decision.idempotencyKey,
      'DECISION_ACTION_IDEMPOTENCY_MISMATCH',
    );
    assertSafeEventPayload({
      preconditionSnapshot: decision.preconditionSnapshot,
      action: decision.action,
    });
    return withTransaction(this.db, () => {
      const existing = this.db
        .prepare('SELECT * FROM supervisor_decisions WHERE idempotency_key=?')
        .get(decision.idempotencyKey) as DecisionRow | undefined;
      if (existing) {
        const durable = this.fromRow(existing);
        const same =
          durable.planId === decision.planId &&
      ...
      this.db
        .prepare(
          'INSERT INTO supervisor_decisions(decision_id,supervisor_id,plan_id,version,observation_cursor,projection_digest,idempotency_key,precondition_snapshot,action_payload,validator_result,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          decision.decisionId,
          decision.supervisorId,
          decision.planId,
          decision.version,
          decision.observationCursor,
          decision.projectionDigest,
          decision.idempotencyKey,
          encode(decision.preconditionSnapshot),
          encode(decision.action),
          decision.validatorResult ? encode(decision.validatorResult) : null,
          decision.createdAt,
        );
      this.events.appendInTransaction(
        makeEvent(decision.decisionId, 'DECISION', 'SUPERVISOR_DECISION_APPENDED', {
          supervisorId: decision.supervisorId,
          planId: decision.planId,
          observationCursor: decision.observationCursor,
  ...
  getById(decisionId: string): SupervisorDecision {
    const row = this.db
      .prepare('SELECT * FROM supervisor_decisions WHERE decision_id=?')
      .get(decisionId) as DecisionRow | undefined;
    if (!row) throw new ForgeFlowError('DECISION_NOT_FOUND');
  ...
  listBySupervisor(supervisorId: string): SupervisorDecision[] {
    const rows = this.db
      .prepare(
        'SELECT * FROM supervisor_decisions WHERE supervisor_id=? ORDER BY created_at,decision_id',
      )
      .all(supervisorId) as unknown as DecisionRow[];
  ...
  listAfterCursor(supervisorId: string, cursor: number, limit = 100): SupervisorDecision[] {
    const rows = this.db
      .prepare(
        'SELECT * FROM supervisor_decisions WHERE supervisor_id=? AND observation_cursor>? ORDER BY observation_cursor,created_at LIMIT ?',
      )
      .all(supervisorId, cursor, limit) as unknown as DecisionRow[];
    return rows.map((row) => this.fromRow(row));
  }
  ...
  private fromRow(row: DecisionRow): SupervisorDecision {
    return {
      ...
      validatorResult: row.validator_result
        ? decode<SupervisorDecision['validatorResult']>(row.validator_result)
        : undefined,
      createdAt: row.created_at,
    };
  }
...
function actionFrom(row: ActionRow): SupervisorAction {
  const result = row.result
    ? decode<NonNullable<SupervisorAction['result']>>(row.result)
    : undefined;
  if (row.execution_id && result && !result.linkedExecutionId)
    result.linkedExecutionId = row.execution_id;
  if (row.child_plan_id && result && !result.linkedPlanId) result.linkedPlanId = row.child_plan_id;
  if (row.pull_request_id && result && !result.pullRequestId)
    result.pullRequestId = row.pull_request_id;
  return {
    ...
    idempotencyKey: row.idempotency_key,
    preconditionSnapshot: decode<Record<string, unknown>>(row.precondition_snapshot),
    payload: decode<SupervisorAction['payload']>(row.payload),
    status: row.status,
    validation: row.validation ? decode<SupervisorAction['validation']>(row.validation) : undefined,
    result,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
...
export class ActionRepository {
  ...
  create(action: SupervisorAction): MutationResult<SupervisorAction> {
    ...
    assertSafeEventPayload({
      preconditionSnapshot: action.preconditionSnapshot,
    ...
    return withTransaction(this.db, () => {
      ...
      this.db
        .prepare(
          'INSERT INTO supervisor_actions(action_id,supervisor_id,plan_id,version,type,idempotency_key,observation_cursor,projection_digest,precondition_snapshot,payload,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
        )
        .run(
          action.actionId,
          action.supervisorId,
          ...
          action.observationCursor,
          action.projectionDigest,
          encode(action.preconditionSnapshot),
      ...
      this.events.appendInTransaction(
        makeEvent(action.actionId, 'ACTION', 'SUPERVISOR_ACTION_CREATED', {
  ...
  getById(actionId: string): SupervisorAction {
    const row = this.db
      .prepare('SELECT * FROM supervisor_actions WHERE action_id=?')
      .get(actionId) as ActionRow | undefined;
    if (!row) throw new ForgeFlowError('ACTION_NOT_FOUND');
    return actionFrom(row);
  }

  findByIdempotencyKey(key: string): SupervisorAction | undefined {
    const row = this.db
      .prepare('SELECT * FROM supervisor_actions WHERE idempotency_key=?')
  ...
  compareAndSetStatus(
    ...
    next: ActionStatus,
  ): MutationResult<SupervisorAction> {
    return withTransaction(this.db, () => {
      ...
      const result = this.db
        .prepare(
          'UPDATE supervisor_actions SET status=?,updated_at=? WHERE action_id=? AND status=?',
        )
        .run(next, iso(), actionId, expected);
      if (Number(result.changes) !== 1)
        return { status: 'rejected', value: this.getById(actionId), reason: 'STALE_STATUS' };
      this.events.appendInTransaction(
        makeEvent(actionId, 'ACTION', 'SUPERVISOR_ACTION_STATUS_CHANGED', {
          from: expected,
          to: next,
        }),
      );
  ...
  recordValidation(
    actionId: string,
    validation: NonNullable<SupervisorAction['validation']>,
  ): MutationResult<SupervisorAction> {
    return withTransaction(this.db, () => {
      const current = this.getById(actionId);
      if (current.validation) {
        if (encode(current.validation) === encode(validation))
          return { status: 'existing', value: current };
        throw new ForgeFlowError('ACTION_VALIDATION_IMMUTABLE');
      }
      if (current.status !== 'PROPOSED' && current.status !== 'VALIDATING')
        throw new ForgeFlowError('ACTION_VALIDATION_STATE_INVALID');
      let transitioned = current;
      if (transitioned.status === 'PROPOSED') {
        transitioned = transitionAction(transitioned, 'VALIDATING', iso());
        this.db
          .prepare('UPDATE supervisor_actions SET status=?,updated_at=? WHERE action_id=?')
          .run('VALIDATING', transitioned.updatedAt, actionId);
        this.events.appendInTransaction(
          makeEvent(actionId, 'ACTION', 'SUPERVISOR_ACTION_STATUS_CHANGED', {
            from: 'PROPOSED',
            to: 'VALIDATING',
      ...
      this.db
        .prepare(
          'UPDATE supervisor_actions SET status=?,validation=?,updated_at=? WHERE action_id=?',
        )
        .run(next, encode(validation), updatedAt, actionId);
      this.events.appendInTransaction(
        makeEvent(actionId, 'ACTION', 'SUPERVISOR_ACTION_VALIDATED', {
  ...
  private attach(
    actionId: string,
    column: 'execution_id' | 'child_plan_id' | 'pull_request_id',
    value: string,
    eventType: string,
  ): MutationResult<SupervisorAction> {
    return withTransaction(this.db, () => {
      ...
      const row = this.db
        .prepare('SELECT ' + column + ' AS value FROM supervisor_actions WHERE action_id=?')
        .get(actionId) as { value: string | null } | undefined;
      const linked = row?.value ?? undefined;
      if (linked === value) return { status: 'existing', value: current };
      if (linked) throw new ForgeFlowError('DURABLE_EVIDENCE_IMMUTABLE');
      this.db
        .prepare('UPDATE supervisor_actions SET ' + column + '=?,updated_at=? WHERE action_id=?')
        .run(value, iso(), actionId);
  ...
  recordSuccess(
    actionId: string,
    result: NonNullable<SupervisorAction['result']>,
  ): MutationResult<SupervisorAction> {
  ...
  private finish(
    ...
    return withTransaction(this.db, () => {
      ...
      if (current.status === nextStatus) {
        if (encode(current.result) === encode(result))
          return { status: 'existing', value: current };
        throw new ForgeFlowError('ACTION_RESULT_IMMUTABLE');
      }
      let transitioned = current;
      if (transitioned.status === 'ACCEPTED' || transitioned.status === 'FAILED') {
        ...
        this.db
          .prepare('UPDATE supervisor_actions SET status=?,updated_at=? WHERE action_id=?')
          .run('EXECUTING', transitioned.updatedAt, actionId);
        this.events.appendInTransaction(
          makeEvent(actionId, 'ACTION', 'SUPERVISOR_ACTION_STATUS_CHANGED', {
            from,
            to: 'EXECUTING',
      ...
      this.db
        .prepare('UPDATE supervisor_actions SET status=?,result=?,updated_at=? WHERE action_id=?')
        .run(nextStatus, encode(result), transitioned.updatedAt, actionId);
      this.events.appendInTransaction(
        makeEvent(actionId, 'ACTION', 'SUPERVISOR_ACTION_FINISHED', {
          status: nextStatus,
          code: result.code,
        }),
      );
...
export class ResourceRepository {
    ...
    return withTransaction(this.db, () => {
      this.db
        ...
        .run(
          ...
          observation.kind,
          observation.status,
          encode(observation.capabilities),
          observation.quotaRemaining ?? null,
          encode(observation),
          observation.observedAt,
          iso(),
        );
      this.events.appendInTransaction(
        makeEvent(observation.resourceId, 'RESOURCE', 'RESOURCE_OBSERVED', {
  ...
  get(resourceId: string): ResourceObservation | undefined {
    const row = this.db
      .prepare('SELECT observation FROM resources WHERE resource_id=?')
      .get(resourceId) as { observation: string } | undefined;
...
function tierFromRank(rank: number): ResourceTier {
  ...
  throw new ForgeFlowError('RESOURCE_TIER_RANK_INVALID', 'Unknown resource tier rank: ' + String(rank));
}
...
export class ExecutionResourceSelectionRepository {
    ...
    return withTransaction(this.db, () => {
      ...
      if (existing) {
        if (!sameSelection(existing, selection))
          throw new ForgeFlowError('EXECUTION_RESOURCE_SELECTION_IMMUTABLE');
        return { status: 'existing', value: existing };
      }
      if (
        !this.db
        ...
        throw new ForgeFlowError('EXECUTION_NOT_FOUND');
      }
      this.db
        .prepare(
  ...
  require(executionId: string): ExecutionResourceSelection {
    ...
    if (!selection) throw new ForgeFlowError('EXECUTION_RESOURCE_SELECTION_NOT_FOUND');
    return selection;
  }
...
function projectPlanLeaseFrom(row: ProjectPlanLeaseRow): ProjectPlanLease {
  const lease: ProjectPlanLease = {
    projectKey: row.project_key,
    repositoryPath: row.repository_path,
  ...
  validateProjectPlanLease(lease);
  return lease;
}

function projectPlanQueueFrom(row: ProjectPlanQueueRow): ProjectPlanQueueEntry {
  return {
    planId: row.plan_id,
    projectKey: row.project_key,
    repositoryPath: row.repository_path,
...
export class ProjectPlanSchedulerRepository {
  constructor(
    readonly db: DatabaseSync,
    readonly events = new EventStore(db),
  ) {}
  ...
  private reconcileActivationRevision(
    ...
    if (plan.status !== 'DRAFT' && plan.status !== 'QUEUED')
      throw new ForgeFlowError('PROJECT_PLAN_ACTIVATION_REVISION_CONFLICT');
    const updatedAt = iso();
    const result = this.db
      .prepare(
        'UPDATE plans SET current_revision=?,updated_at=? WHERE plan_id=? AND current_revision=? AND status=?',
    ...
    if (Number(result.changes) !== 1) throw new ForgeFlowError('PROJECT_PLAN_ACTIVATION_REVISION_STALE');
    this.events.appendInTransaction(
  ...
  getLease(projectKey: string): ProjectPlanLease | undefined {
    ...
    const row = this.db
      .prepare('SELECT * FROM project_plan_leases WHERE project_key=?')
      .get(projectKey) as ProjectPlanLeaseRow | undefined;
    return row ? projectPlanLeaseFrom(row) : undefined;
  }

  listLeases(): ProjectPlanLease[] {
    const rows = this.db
      .prepare('SELECT * FROM project_plan_leases ORDER BY project_key')
      .all() as unknown as ProjectPlanLeaseRow[];
  ...
  getQueueEntry(planId: string): ProjectPlanQueueEntry | undefined {
    ...
    return row ? projectPlanQueueFrom(row) : undefined;
  }

  listQueue(projectKey: string, includeInactive = false): ProjectPlanQueueEntry[] {
    failClosed(projectKey.trim().length > 0, 'PROJECT_PLAN_PROJECT_REQUIRED');
    const rows = (includeInactive
      ? this.db
          .prepare(
            'SELECT * FROM project_plan_queue WHERE project_key=? ORDER BY priority DESC,sequence ASC',
          )
          .all(projectKey)
      : this.db
          .prepare(
            'SELECT * FROM project_plan_queue WHERE project_key=? AND activated_at IS NULL AND cancelled_at IS NULL ORDER BY priority DESC,sequence ASC',
          )
          .all(projectKey)) as unknown as ProjectPlanQueueRow[];
    return rows.map(projectPlanQueueFrom);
  }

  scheduleRootPlan(planId: string, priority = 0): RootPlanScheduleResult {
    validateProjectPlanPriority(priority);
    return withTransaction(this.db, () => {
      const plans = new PlanRepository(this.db, this.events);
      let plan = plans.getPlan(planId);
      if (plan.parentPlanId) throw new ForgeFlowError('PROJECT_PLAN_ROOT_REQUIRED');
      const at = iso();
      const row = this.db
        .prepare('SELECT * FROM project_plan_leases WHERE project_key=?')
        .get(plan.projectKey) as ProjectPlanLeaseRow | undefined;
      if (row && row.repository_path !== plan.repositoryPath)
        throw new ForgeFlowError('PROJECT_PLAN_REPOSITORY_MISMATCH');
      ...
      if (!row || row.active_root_plan_id === null) {
        ...
        if (row) {
          ...
          if (Number(updated.changes) !== 1) throw new ForgeFlowError('PROJECT_PLAN_LEASE_STALE');
        } else {
          this.db
            .prepare(
        ...
        if (plan.status === 'DRAFT' || plan.status === 'QUEUED') {
          ...
          this.db
            .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=? AND status=?')
            .run('READY', at, plan.planId, plan.status);
          this.events.appendInTransaction(
            makeEvent(plan.planId, 'PLAN', 'PLAN_STATUS_CHANGED', {
              from: plan.status,
              to: 'READY',
            }),
          );
          plan = next;
        } else if (plan.status !== 'READY' && plan.status !== 'RUNNING') {
          throw new ForgeFlowError('PROJECT_PLAN_ACTIVATION_STATE_INVALID');
        }
        this.db
          .prepare(
            'UPDATE project_plan_queue SET activated_at=COALESCE(activated_at,?) WHERE plan_id=? AND cancelled_at IS NULL',
          )
          .run(at, planId);
        this.events.appendInTransaction(
          makeEvent(plan.planId, 'PLAN', 'PROJECT_PLAN_LEASE_ACQUIRED', {
      ...
      if (row.active_root_plan_id === planId) {
        ...
        return { status: 'ACTIVE', lease: projectPlanLeaseFrom(row) };
      }

      if (plan.status !== 'DRAFT' && plan.status !== 'QUEUED')
        throw new ForgeFlowError('PROJECT_PLAN_QUEUE_STATE_INVALID');
      let queued = this.getQueueEntry(planId);
      if (queued) {
        if (
          queued.projectKey !== plan.projectKey ||
          queued.repositoryPath !== plan.repositoryPath ||
          queued.cancelledAt ||
          queued.activatedAt
        )
          throw new ForgeFlowError('PROJECT_PLAN_QUEUE_CONFLICT');
        if (queued.priority !== priority) {
          this.db
            .prepare('UPDATE project_plan_queue SET priority=? WHERE plan_id=?')
            .run(priority, planId);
          queued = this.getQueueEntry(planId)!;
        }
      } else {
        const sequenceRow = this.db
          .prepare(
            'SELECT COALESCE(MAX(sequence),0)+1 AS sequence FROM project_plan_queue WHERE project_key=?',
          )
          .get(plan.projectKey) as { sequence: number };
        this.db
          .prepare(
            'INSERT INTO project_plan_queue(plan_id,project_key,repository_path,sequence,priority,enqueued_at) VALUES(?,?,?,?,?,?)',
          )
          .run(
            planId,
            plan.projectKey,
      ...
      if (plan.status === 'DRAFT') {
        transitionPlan(plan, 'QUEUED', at);
        this.db
          .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=?')
          .run('QUEUED', at, planId);
        this.events.appendInTransaction(
          makeEvent(plan.planId, 'PLAN', 'PLAN_STATUS_CHANGED', { from: 'DRAFT', to: 'QUEUED' }),
        );
      }
      this.events.appendInTransaction(
        makeEvent(plan.planId, 'PLAN', 'PROJECT_PLAN_QUEUED', {
          projectKey: plan.projectKey,
          sequence: queued.sequence,
          priority: queued.priority,
        }),
      );
      return { status: 'QUEUED', lease: projectPlanLeaseFrom(row), queueEntry: queued };
    });
  }
  ...
  tryAcquire(
    ...
    return withTransaction(this.db, () => {
      const plan = new PlanRepository(this.db, this.events).getPlan(rootPlanId);
      if (plan.parentPlanId || plan.projectKey !== projectKey)
        throw new ForgeFlowError('PROJECT_PLAN_ROOT_REQUIRED');
      const current = this.getLease(projectKey);
      if (!current) {
        if (expectedVersion !== 0) return { status: 'rejected', reason: 'STALE_VERSION' };
        const at = iso();
        this.db
          .prepare(
            'INSERT INTO project_plan_leases(project_key,repository_path,active_root_plan_id,committed_revision,version,acquired_at,updated_at) VALUES(?,?,?,?,?,?,?)',
      ...
      if (current.version !== expectedVersion)
        return { status: 'rejected', value: current, reason: 'STALE_VERSION' };
      if (current.activeRootPlanId) {
        return current.activeRootPlanId === rootPlanId
          ? { status: 'existing', value: current }
  ...
  renew(
    ...
    return withTransaction(this.db, () => {
      const current = this.getLease(projectKey);
      if (!current) return { status: 'rejected', reason: 'PROJECT_PLAN_LEASE_NOT_FOUND' };
      if (current.activeRootPlanId !== rootPlanId)
        return { status: 'rejected', value: current, reason: 'PROJECT_PLAN_LEASE_OWNER_MISMATCH' };
      if (current.version !== expectedVersion)
        return { status: 'rejected', value: current, reason: 'STALE_VERSION' };
      const at = iso();
      const result = this.db
        .prepare(
          'UPDATE project_plan_leases SET version=version+1,updated_at=? WHERE project_key=? AND active_root_plan_id=? AND version=?',
        )
  ...
  reprioritize(planId: string, priority: number): MutationResult<ProjectPlanQueueEntry> {
    validateProjectPlanPriority(priority);
    return withTransaction(this.db, () => {
      const current = this.getQueueEntry(planId);
      if (!current) return { status: 'rejected', reason: 'PROJECT_PLAN_QUEUE_NOT_FOUND' };
      if (current.activatedAt || current.cancelledAt)
        return { status: 'rejected', value: current, reason: 'PROJECT_PLAN_QUEUE_INACTIVE' };
      if (current.priority === priority) return { status: 'existing', value: current };
      this.db
        .prepare('UPDATE project_plan_queue SET priority=? WHERE plan_id=?')
        .run(priority, planId);
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PROJECT_PLAN_REPRIORITIZED', {
          from: current.priority,
          to: priority,
        }),
      );
      return { status: 'updated', value: this.getQueueEntry(planId)! };
    });
  }

  cancelQueued(planId: string): MutationResult<ProjectPlanQueueEntry> {
    return withTransaction(this.db, () => {
      const plans = new PlanRepository(this.db, this.events);
      ...
      if (current.activatedAt)
        return { status: 'rejected', value: current, reason: 'PROJECT_PLAN_ALREADY_ACTIVE' };
      if (current.cancelledAt) return { status: 'existing', value: current };
      if (plan.status !== 'QUEUED') throw new ForgeFlowError('PROJECT_PLAN_QUEUE_STATE_INVALID');
      const at = iso();
      transitionPlan(plan, 'CANCELLED', at);
      this.db
        .prepare('UPDATE project_plan_queue SET cancelled_at=? WHERE plan_id=?')
        .run(at, planId);
      this.db
        .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=?')
        .run('CANCELLED', at, planId);
      this.events.appendInTransaction(
        makeEvent(planId, 'PLAN', 'PLAN_STATUS_CHANGED', { from: 'QUEUED', to: 'CANCELLED' }),
      );
      return { status: 'updated', value: this.getQueueEntry(planId)! };
    });
  }

  claimNext(projectKey: string, expectedVersion: number): MutationResult<ProjectPlanLease> {
    return withTransaction(this.db, () => {
      const current = this.getLease(projectKey);
      if (!current) return { status: 'rejected', reason: 'PROJECT_PLAN_LEASE_NOT_FOUND' };
      if (current.version !== expectedVersion)
      ...
      if (current.activeRootPlanId)
        return { status: 'rejected', value: current, reason: 'PROJECT_PLAN_LEASE_HELD' };
      const next = this.nextQueued(projectKey);
      if (!next) return { status: 'existing', value: current };
      return this.activateQueuedInTransaction(current, next);
    });
  }

  releaseAndActivateNext(
    projectKey: string,
    rootPlanId: string,
    expectedVersion: number,
    ...
    return withTransaction(this.db, () => {
      const current = this.getLease(projectKey);
      if (!current) throw new ForgeFlowError('PROJECT_PLAN_LEASE_NOT_FOUND');
      if (current.activeRootPlanId !== rootPlanId)
        throw new ForgeFlowError('PROJECT_PLAN_LEASE_OWNER_MISMATCH');
      if (current.version !== expectedVersion) throw new ForgeFlowError('PROJECT_PLAN_LEASE_STALE');
      const released = new PlanRepository(this.db, this.events).getPlan(rootPlanId);
      if (!isTerminalPlanStatus(released.status))
        throw new ForgeFlowError('PROJECT_PLAN_RELEASE_REQUIRES_TERMINAL');
      const next = this.nextQueued(projectKey);
      const at = iso();
      if (!next) {
        const result = this.db
        ...
        if (Number(result.changes) !== 1) throw new ForgeFlowError('PROJECT_PLAN_LEASE_STALE');
        this.events.appendInTransaction(
      ...
      if (nextPlan.status !== 'QUEUED') throw new ForgeFlowError('PROJECT_PLAN_QUEUE_STATE_INVALID');
      ...
      this.db
        .prepare('UPDATE plans SET status=?,updated_at=? WHERE plan_id=? AND status=?')
        .run('READY', at, next.planId, 'QUEUED');
      this.db
        .prepare(
          'UPDATE project_plan_queue SET activated_at=? WHERE plan_id=? AND activated_at IS NULL AND cancelled_at IS NULL',
        )
        .run(at, next.planId);
      this.events.appendInTransaction(
        makeEvent(rootPlanId, 'PLAN', 'PROJECT_PLAN_LEASE_RELEASED', {
      ...
      this.events.appendInTransaction(
        makeEvent(next.planId, 'PLAN', 'PLAN_STATUS_CHANGED', { from: 'QUEUED', to: 'READY' }),
      );
      this.events.appendInTransaction(
      ...
      return {
        releasedPlanId: rootPlanId,
        activatedPlanId: next.planId,
        lease: this.getLease(projectKey)!,
      };
    });
  }

  private nextQueued(projectKey: string): ProjectPlanQueueEntry | undefined {
    const row = this.db
      .prepare(
        "SELECT queue.* FROM project_plan_queue queue JOIN plans ON plans.plan_id=queue.plan_id WHERE queue.project_key=? AND queue.activated_at IS NULL AND queue.cancelled_at IS NULL AND plans.status='QUEUED' ORDER BY queue.priority DESC,queue.sequence ASC LIMIT 1",
      )
      .get(projectKey) as ProjectPlanQueueRow | undefined;
    return row ? projectPlanQueueFrom(row) : undefined;
  }

  private activateQueuedInTransaction(
    current: ProjectPlanLease,
    next: ProjectPlanQueueEntry,
  ): MutationResult<ProjectPlanLease> {
    if (next.repositoryPath !== current.repositoryPath)
      throw new ForgeFlowError('PROJECT_PLAN_REPOSITORY_MISMATCH');
    const at = iso();
...
export class RelationshipRepository implements PlanRelationshipRepository {
  ...
  createParentChild(input: {
    ...
    return withTransaction(this.db, () => {
      ...
      const parent = this.db
        .prepare('SELECT plan_id FROM plans WHERE plan_id=?')
        .get(input.parentPlanId);
      const child = this.db
        .prepare('SELECT plan_id FROM plans WHERE plan_id=?')
        .get(input.childPlanId);
      if (!parent || !child) throw new ForgeFlowError('PLAN_NOT_FOUND');
      const rel = {
        relationshipId: input.relationshipId ?? id('relationship'),
      ...
      this.db
        ...
        .run(rel.relationshipId, rel.parentPlanId, rel.childPlanId, rel.kind, iso());
      this.events.appendInTransaction(
        makeEvent(rel.relationshipId, 'RELATIONSHIP', 'PARENT_CHILD_CREATED', rel),
      );
  ...
  getParent(childPlanId: string): string | undefined {
    const row = this.db
      .prepare('SELECT parent_plan_id FROM plan_relationships WHERE child_plan_id=?')
      .get(childPlanId) as { parent_plan_id: string } | undefined;
  ...
  getChildren(parentPlanId: string): string[] {
    const rows = this.db
      ...
      .all(parentPlanId) as unknown as Array<{ child_plan_id: string }>;
    return rows.map((row) => row.child_plan_id);
  }
  rejectSelfCycle(parentPlanId: string, childPlanId: string): void {
    if (parentPlanId === childPlanId) throw new ForgeFlowError('PARENT_CHILD_SELF_CYCLE');
    const seen = new Set<string>();
    const visit = (planId: string): void => {
      if (planId === parentPlanId) throw new ForgeFlowError('PARENT_CHILD_CYCLE');
  ...
  rejectDuplicateRelationship(parentPlanId: string, childPlanId: string, kind: string): void {
    const row = this.db
      .prepare(
        'SELECT relationship_id FROM plan_relationships WHERE parent_plan_id=? AND child_plan_id=? AND kind=?',
      )
      .get(parentPlanId, childPlanId, kind);
...
export function createRepositories(db: DatabaseSync): ForgeFlowRepositories {
