...
import { DuplicateKeyError, ForgeFlowError } from '../domain/errors.js';
...
function fromRow(row: EventRow): EventEnvelope {
  let payload: unknown;
  try { payload = JSON.parse(row.payload) as unknown; } catch (error) { throw new ForgeFlowError('CORRUPTED_EVENT_PAYLOAD', 'Cannot decode event ' + row.event_id, error); }
...
export class EventStore {
  ...
  appendInTransaction<T>(event: EventToAppend<T>): EventEnvelope<T> {
    ...
    if (event.sequence !== undefined && event.sequence !== sequence) throw new ForgeFlowError('EVENT_SEQUENCE_CONFLICT');
  ...
  listRecentByAggregate(aggregateId: string, limit = 500): EventEnvelope[] {
    const bounded = Math.max(1, Math.min(limit, 5_000));
    const rows = this.db.prepare('SELECT event_order,event_id,aggregate_id,aggregate_type,sequence,type,payload,occurred_at,correlation_id FROM events WHERE aggregate_id=? ORDER BY sequence DESC LIMIT ?').all(aggregateId, bounded) as unknown as EventRow[];
    rows.reverse();
    return rows.map(fromRow);
  }
