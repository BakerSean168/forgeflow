...
function decodeJson<T>(value: string, code: string): T {
  try { return JSON.parse(value) as T; } catch (error) { throw new ForgeFlowError(code, 'Cannot decode projection data', error); }
}
...
export function buildSupervisorProjection(db: DatabaseSync, supervisorId: string, options: { afterCursor?: number; maxEvents?: number; maxItems?: number } = {}): BoundedSupervisorProjection {
  ...
  if (!supervisor) throw new ForgeFlowError('SUPERVISOR_NOT_FOUND');
  ...
  const base = {
    ...
    supervisor: { supervisorId: supervisor.supervisor_id, status: supervisor.status, observationCursor: supervisor.observation_cursor, allowedActions: ['NO_ACTION', 'CREATE_EXECUTION', 'CONTINUE_EXECUTION', 'RETRY_EXECUTION', 'SWITCH_ROUTE', 'REQUEST_REVIEW', 'CREATE_REPAIR', 'REPLAN_REMAINDER', 'CREATE_CHILD_PLAN', 'PAUSE_FOR_RESOURCE', 'PARK_EXTERNAL_GATE', 'ESCALATE'] },
