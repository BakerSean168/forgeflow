import { ForgeFlowError, failClosed } from './errors.js';
...
function assertPayloadValue(value: unknown, path: string, seen = new WeakSet<object>()): void {
  if (value === undefined || typeof value === 'function' || typeof value === 'symbol' || typeof value === 'bigint') {
    throw new ForgeFlowError('UNSAFE_EVENT_PAYLOAD', 'Unsupported event payload value at ' + path);
  }
  if (value !== null && typeof value === 'object') {
    if (seen.has(value)) throw new ForgeFlowError('UNSAFE_EVENT_PAYLOAD', 'Cyclic event payload at ' + path);
    seen.add(value);
    if (Array.isArray(value)) {
      ...
      for (const [key, child] of Object.entries(value)) {
        if (FORBIDDEN_PAYLOAD_KEY.test(key)) {
          throw new ForgeFlowError('UNSAFE_EVENT_PAYLOAD', 'Forbidden event payload key at ' + path + '.' + key);
        }
...
export function assertSafeEventPayload(value: unknown): void {
  ...
  try {
    ...
    throw new ForgeFlowError('UNSAFE_EVENT_PAYLOAD', 'Event payload is not JSON serializable', error);
  }
}
