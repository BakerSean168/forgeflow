...
function validateAffinityList(affinities: readonly ModelAffinity[], codePrefix: string): void {
  ...
  for (const affinity of affinities) {
    ...
    if (families.has(affinity.modelFamily)) throw new ForgeFlowError(codePrefix + '_DUPLICATE_MODEL');
    const rankKey = affinity.capability + ':' + String(affinity.modelRank);
    if (ranks.has(rankKey)) throw new ForgeFlowError(codePrefix + '_DUPLICATE_RANK');
...
export function validateAffinityPolicy(policy: AffinityPolicy): void {
  if (policy === null || typeof policy !== 'object') throw new ForgeFlowError('AFFINITY_POLICY_INVALID');
...
export function validateExecutionResource(resource: ExecutionResource): void {
  ...
  for (const binding of resource.bindings) {
    ...
    if (bindingIds.has(binding.bindingId)) throw new ForgeFlowError('RESOURCE_BINDING_DUPLICATE');
