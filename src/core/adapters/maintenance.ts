...
function decodeEvidence(value: string): string[] {
  try {
    ...
    throw new ForgeFlowError('CORRUPTED_CANDIDATE_EVIDENCE', 'Candidate evidence is not a non-empty string array.', error);
  }
}
...
export class MaintenanceCandidateRegistry {
  ...
  create(
    ...
    return withTransaction(this.db, () => {
      ...
      if (durableProgram) {
        ...
        if (durableProgram.status !== 'ACTIVE') throw new ForgeFlowError('MAINTENANCE_PROGRAM_DISABLED');
      } else {
  ...
  attachPlan(candidateId: string, planId: string): ImprovementCandidate {
    ...
    return withTransaction(this.db, () => {
      ...
      if (!candidate) throw new ForgeFlowError('CANDIDATE_NOT_FOUND');
      if (candidate.planId === planId) return candidate;
      if (candidate.planId) throw new ForgeFlowError('CANDIDATE_PLAN_IMMUTABLE');
      if (!this.db.prepare('SELECT plan_id FROM plans WHERE plan_id=?').get(planId)) throw new ForgeFlowError('PLAN_NOT_FOUND');
