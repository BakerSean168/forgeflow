...
export class SupervisorActionExecutor {
  ...
  async execute(decision: SupervisorDecision, projection: SupervisorProjection): Promise<ActionExecutionResult> {
    ...
    try {
      ...
      if (result.code === 'KERNEL_PORT_UNAVAILABLE') throw new ForgeFlowError(result.code);
      ...
      const code = error instanceof ForgeFlowError ? error.code : 'ACTION_EXECUTION_FAILED';
