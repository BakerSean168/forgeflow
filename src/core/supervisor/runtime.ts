...
export class HttpSupervisorDecisionClient implements SupervisorDecisionClient {
  ...
  async decide(input: SupervisorDecisionInput): Promise<string> {
    const response = await this.fetchImpl(this.endpoint, {
      ...
      signal: AbortSignal.timeout(this.timeoutMs),
      body: JSON.stringify({ protocol: 'FORGEFLOW_SUPERVISOR_DECISION_V1', conversationId: input.conversationId, supervisorId: input.supervisorId, planId: input.planId, projection: input.projection }),
    });
    if (!response.ok) throw new ForgeFlowError('SUPERVISOR_MODEL_UNAVAILABLE', 'Supervisor model HTTP ' + response.status);
    const payload = await response.text();
    if (payload.length === 0 || payload.length > 64_000) throw new ForgeFlowError('SUPERVISOR_DECISION_INVALID');
    return payload;
  }
}
...
export class SupervisorRuntime {
  ...
  private async runOne(request: WakeRequest): Promise<SupervisorRunResult> {
    ...
    try {
      ...
      if (!planState) throw new ForgeFlowError('PLAN_NOT_FOUND');
      ...
      try {
        ...
        if (swapped.status === 'rejected') throw new ForgeFlowError('SUPERVISOR_CONVERSATION_RECOVERY_RACE');
      ...
      else if (persistedConversation !== conversation.conversationId) throw new ForgeFlowError('SUPERVISOR_CONVERSATION_RECOVERY_RACE');
      if (!this.client) throw new ForgeFlowError('SUPERVISOR_MODEL_UNAVAILABLE');
