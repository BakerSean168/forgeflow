...
function skipString(input: string, index: number): number {
  ...
  throw new ForgeFlowError('DECISION_JSON_INVALID');
}
...
function skipValue(input: string, index: number): number {
  ...
  if (input[index] === '{') {
    ...
    while (index < input.length) {
      index = skipWhitespace(input, index);
      if (input[index] !== '"') throw new ForgeFlowError('DECISION_JSON_INVALID');
      const start = index;
      index = skipString(input, index);
      const key = JSON.parse(input.slice(start, index)) as string;
      if (keys.has(key)) throw new ForgeFlowError('DECISION_DUPLICATE_KEY', 'Duplicate JSON key: ' + key);
      keys.add(key);
      index = skipWhitespace(input, index);
      if (input[index] !== ':') throw new ForgeFlowError('DECISION_JSON_INVALID');
  ...
  if (index === start) throw new ForgeFlowError('DECISION_JSON_INVALID');
  return index;
}

function assertNoDuplicateKeys(input: string): void {
  const end = skipValue(input, 0);
  if (skipWhitespace(input, end) !== input.length) throw new ForgeFlowError('DECISION_JSON_INVALID');
}
...
function text(value: unknown, code: string, max = MAX_STRING): string {
  if (typeof value !== 'string' || value.trim().length === 0 || value.length > max) throw new ForgeFlowError(code);
  return value;
}
...
function parsePayload(type: SupervisorActionType, value: unknown): SupervisorActionPayload {
  ...
  if (payload.type !== type) throw new ForgeFlowError('ACTION_PAYLOAD_TYPE_MISMATCH');
  switch (type) {
    ...
    case 'CREATE_REPAIR': {
      ...
      if (!Array.isArray(refs) || refs.length > 20 || !refs.every((ref) => typeof ref === 'string' && ref.length <= 300)) throw new ForgeFlowError('REPAIR_FINDINGS_INVALID');
    ...
    case 'REPLAN_REMAINDER': {
      if (!Array.isArray(payload.workItems) || payload.workItems.length > 100) throw new ForgeFlowError('REPLAN_ITEMS_INVALID');
      const workItems = payload.workItems.map((item) => {
        ...
        if (!Array.isArray(dependencies) || !dependencies.every((entry) => typeof entry === 'string' && entry.length <= 200)) throw new ForgeFlowError('REPLAN_DEPENDENCIES_INVALID');
        if (!Array.isArray(acceptanceCriteria) || !acceptanceCriteria.every((entry) => typeof entry === 'string' && entry.length <= 2000)) throw new ForgeFlowError('REPLAN_ACCEPTANCE_INVALID');
    ...
    case 'CREATE_CHILD_PLAN':
      if (payload.relation !== 'SYSTEM_REPAIR' && payload.relation !== 'INFRASTRUCTURE_REPAIR' && payload.relation !== 'FOLLOW_UP') throw new ForgeFlowError('CHILD_RELATION_INVALID');
    ...
    case 'PARK_EXTERNAL_GATE':
      if (payload.gate !== 'NATIVE_MACHINE' && payload.gate !== 'SECRET' && payload.gate !== 'HUMAN' && payload.gate !== 'POLICY') throw new ForgeFlowError('EXTERNAL_GATE_INVALID');
      return { type, gate: payload.gate, reason: text(payload.reason, 'ACTION_REASON_REQUIRED') };
    case 'ESCALATE':
      if (payload.severity !== 'LOW' && payload.severity !== 'MEDIUM' && payload.severity !== 'HIGH') throw new ForgeFlowError('ESCALATION_SEVERITY_INVALID');
...
export function parseSupervisorDecision(raw: string, now = new Date().toISOString()): SupervisorDecision {
  if (Buffer.byteLength(raw, 'utf8') > MAX_BYTES) throw new ForgeFlowError('DECISION_TOO_LARGE');
  assertNoDuplicateKeys(raw);
  let parsed: unknown;
  try { parsed = JSON.parse(raw) as unknown; } catch (error) { throw new ForgeFlowError('DECISION_JSON_INVALID', 'Decision must be one JSON object', error); }
  const root = asRecord(parsed, 'DECISION_OBJECT_REQUIRED');
  assertSafeEventPayload(root);
  if (root.version !== SUPERVISOR_PROTOCOL_VERSION && root.version !== 'FORGEFLOW_SUPERVISOR_DECISION_V1') throw new ForgeFlowError('DECISION_VERSION_UNSUPPORTED', 'version=' + typeof root.version + ':' + String(root.version).slice(0, 40) + ';keys=' + Object.keys(root).sort().join(','));
  ...
  if (typeof root.observationCursor !== 'number' || !Number.isInteger(root.observationCursor) || root.observationCursor < 0) throw new ForgeFlowError('DECISION_CURSOR_INVALID');
  ...
  if (!isActionType(actionRoot.type)) throw new ForgeFlowError('ACTION_TYPE_UNSUPPORTED');
