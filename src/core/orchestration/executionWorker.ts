...
function localExecutionBlockerCode(
  ...
  if (
    /HEADLESS_WORKER_WRITABLE_ROOT_NOT_ALLOWED|FORGEFLOW_(?:IMPLEMENTATION|REVIEW)_EVIDENCE_PATH_INVALID/i.test(
      detail,
    )
  )
...
export class ExecutionWorker {
  ...
  constructor(
    ...
    for (const entry of routes) {
      if (!entry.route.trim()) throw new ForgeFlowError('EXECUTION_ROUTE_REQUIRED');
      if (mapped.has(entry.route)) throw new ForgeFlowError('EXECUTION_ROUTE_DUPLICATE');
    ...
    if (
      ...
      throw new ForgeFlowError('EXECUTION_CYCLE_LIMIT_INVALID');
    }
    if (this.requireResourceSelection && !this.providerFactory)
      throw new ForgeFlowError('EXECUTION_PROVIDER_FACTORY_REQUIRED');
    if (
  ...
  private resolveProvider(
    ...
    if (selection) {
      if (selection.phase !== execution.identity.phase)
        throw new ForgeFlowError('EXECUTION_RESOURCE_SELECTION_PHASE_MISMATCH');
      if (!this.providerFactory) throw new ForgeFlowError('EXECUTION_PROVIDER_FACTORY_REQUIRED');
  ...
  async replaceStalledProviderSession(
    ...
    try {
      ...
      if (!idempotencyKey.trim() || idempotencyKey.length > 1_000)
        throw new ForgeFlowError('PROVIDER_REPLACEMENT_IDEMPOTENCY_REQUIRED');
      if (!instruction.trim() || instruction.length > 32_000)
        throw new ForgeFlowError('PROVIDER_REPLACEMENT_INSTRUCTION_INVALID');
      if (!reason.trim() || reason.length > 2_000)
        throw new ForgeFlowError('PROVIDER_SESSION_REPLACEMENT_INVALID');
      const resolved = this.resolveProvider(execution);
      if (!resolved)
        return { executionId, status: 'WAITING', code: 'EXECUTION_RESOURCE_SELECTION_UNAVAILABLE' };
      const { provider, selection } = resolved;
      ...
      if (existingEvidence) {
        ...
        if (
          ...
          throw new ForgeFlowError('PROVIDER_REPLACEMENT_EVIDENCE_INVALID');
        let session = this.repositories.sessions.get(executionId);
        if (session.providerSessionId === previous) {
          ...
          if (swapped.status === 'rejected' || !swapped.value)
            throw new ForgeFlowError(swapped.reason ?? 'STALE_PROVIDER_SESSION');
          session = this.repositories.sessions.get(executionId);
        } else if (session.providerSessionId !== replacementId) {
          throw new ForgeFlowError('PROVIDER_REPLACEMENT_IDEMPOTENCY_CONFLICT');
        }
      ...
      if (replacement.providerSessionId === previousProviderSessionId)
        throw new ForgeFlowError('PROVIDER_REPLACEMENT_SESSION_UNCHANGED');
  ...
  async adoptPausedImplementation(
    ...
    try {
      ...
      if (execution.identity.phase !== 'IMPLEMENT' && execution.identity.phase !== 'IMPLEMENT_FIX')
        throw new ForgeFlowError('OPERATOR_ADOPTION_IMPLEMENTATION_ONLY');
      if (!idempotencyKey.trim() || idempotencyKey.length > 1_000)
        throw new ForgeFlowError('OPERATOR_ADOPTION_IDEMPOTENCY_REQUIRED');
      if (!reason.trim() || reason.length > 2_000)
        throw new ForgeFlowError('OPERATOR_ADOPTION_REASON_INVALID');
      const resolved = this.resolveProvider(execution);
      if (!resolved)
        return { executionId, status: 'WAITING', code: 'EXECUTION_RESOURCE_SELECTION_UNAVAILABLE' };
  ...
  async abortPausedProviderAttempt(
    ...
    try {
      ...
      if (!idempotencyKey.trim() || idempotencyKey.length > 1_000)
        throw new ForgeFlowError('PROVIDER_ABORT_IDEMPOTENCY_REQUIRED');
      if (!reason.trim() || reason.length > 2_000)
        throw new ForgeFlowError('PROVIDER_ABORT_REASON_INVALID');
      const resolved = this.resolveProvider(execution);
      if (!resolved)
        return { executionId, status: 'WAITING', code: 'EXECUTION_RESOURCE_SELECTION_UNAVAILABLE' };
      const { provider } = resolved;
  ...
  async runExecution(executionId: string): Promise<ExecutionWorkerResult> {
    ...
    try {
      ...
      if (!execution.identity.sourceRevision)
        throw new ForgeFlowError('EXECUTION_SOURCE_REVISION_REQUIRED');
      ...
      if (
        ...
        throw new ForgeFlowError('EXECUTION_SESSION_ROUTE_MISMATCH');
      }
      if (!session) {
        ...
        if (!session) throw new ForgeFlowError('EXECUTION_SESSION_CREATE_FAILED');
        this.repositories.evidence.append({
          executionId,
          kind: 'WORKSPACE',
      ...
      session = this.repositories.sessions.get(executionId);
      if (!session.providerSessionId) throw new ForgeFlowError('PROVIDER_SESSION_ID_REQUIRED');
  ...
  private async handleMeaningfulProgress(
    ...
    try {
      ...
      if (!(error instanceof ForgeFlowError) || error.code !== 'WORKSPACE_GIT_COMMAND_FAILED') throw error;
  ...
  private async recoverMeaningfulProgressStall(
    ...
    const failure = new ForgeFlowError(
  ...
  private assertProviderPhase(provider: ExecutionProviderPort, execution: Execution): void {
    if (execution.identity.phase === 'REVIEW') {
      if ((provider as Partial<ReviewProviderPort>).independentReview !== true)
        throw new ForgeFlowError('INDEPENDENT_REVIEW_PROVIDER_REQUIRED');
    } else if ((provider as Partial<ReviewProviderPort>).independentReview === true) {
      throw new ForgeFlowError('IMPLEMENTATION_PROVIDER_REQUIRED');
    }
  }
  ...
  private async resolveProviderSnapshot(
    ...
    if (attached.status === 'rejected')
      throw new ForgeFlowError(attached.reason ?? 'STALE_PROVIDER_SESSION');
    return snapshot;
  }
  ...
  private recordActiveProviderStatus(
    ...
    if (session.providerStatus === snapshot.status) {
      ...
      if (heartbeat.status === 'rejected' && heartbeat.reason !== 'STALE_PROVIDER_HEARTBEAT')
        throw new ForgeFlowError(heartbeat.reason ?? 'PROVIDER_HEARTBEAT_REJECTED');
      return;
    }
    ...
    if (updated.status === 'rejected') throw new ForgeFlowError(updated.reason ?? 'STALE_PROVIDER_STATUS');
  }
  ...
  private recordTerminalProviderStatus(
    ...
    if (completed.status === 'rejected')
      throw new ForgeFlowError(completed.reason ?? 'STALE_PROVIDER_STATUS');
  }
  ...
  private persistReviewVerdict(executionId: string, evidence: CompletionEvidence): void {
    if (evidence.phase !== 'REVIEW') throw new ForgeFlowError('WORKSPACE_REVIEW_EVIDENCE_INVALID');
  ...
  private renewLease(executionId: string, leaseToken: string): void {
    ...
    if (!renewed.value || renewed.status === 'rejected')
      throw new ForgeFlowError(renewed.reason ?? 'EXECUTION_LEASE_LOST');
  }
}
