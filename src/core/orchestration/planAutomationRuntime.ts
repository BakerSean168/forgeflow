...
function normalizePolicy(
  ...
  if (!resourceSelectionEnabled && implementationRoutes.length === 0)
    throw new ForgeFlowError('IMPLEMENTATION_ROUTE_REQUIRED');
  if (!resourceSelectionEnabled && reviewRoutes.length === 0)
    throw new ForgeFlowError('REVIEW_ROUTE_REQUIRED');
  ...
  for (const [name, value] of Object.entries({
    ...
    if (!Number.isInteger(value) || value < 1 || value > 20)
      throw new ForgeFlowError('PLAN_AUTOMATION_LIMIT_INVALID', name);
  }
  return {
...
export class PlanAutomationRuntime {
  ...
  private ensurePlanRunning(plan: Plan): Plan {
    ...
    if (!resumed.value || resumed.status === 'rejected')
      throw new ForgeFlowError(resumed.reason ?? 'STALE_PLAN_STATUS');
    ...
    for (const plan of unique.values()) {
      try {
        ...
        results.push({
          planId: plan.planId,
          status: 'WAITING',
          code: error instanceof ForgeFlowError ? error.code : 'PLAN_AUTOMATION_CYCLE_FAILED',
        });
      }
    }
    return results;
  ...
  async runPlan(planId: string): Promise<PlanAutomationResult> {
    try {
      ...
      if (!(error instanceof ForgeFlowError) || error.code !== 'NO_ELIGIBLE_RESOURCE') throw error;
  ...
  async reconcilePlan(planId: string, mode = 'auto'): Promise<PlanAutomationResult> {
    ...
    if (failedItems.length !== 1) throw new ForgeFlowError('PLAN_RECOVERY_WORK_ITEM_AMBIGUOUS');
    ...
    if (active) {
      if (active.identity.workItemId !== item.workItemId)
        throw new ForgeFlowError('PLAN_RECOVERY_ACTIVE_EXECUTION_CONFLICT');
    ...
    if (latestReview?.status === 'PASSED') {
      ...
      if (
        ...
        throw new ForgeFlowError('PLAN_REVIEWED_CANDIDATE_INVALID');
      }
      this.reviveFailedPlan(plan, item);
    ...
    if (!candidate) throw new ForgeFlowError('PLAN_RECOVERY_EXECUTION_MISSING');
    if (
      candidate.status !== 'FAILED' &&
      candidate.status !== 'BLOCKED' &&
      candidate.status !== 'CANCELLED'
    )
      throw new ForgeFlowError('PLAN_RECOVERY_EXECUTION_INVALID');
    ...
    if (candidate.identity.phase === 'IMPLEMENT') {
      ...
      const repairRoot = candidate.identity.parentExecutionId
        ...
        : undefined;
      if (!repairRoot?.resultRevision) throw new ForgeFlowError('REPAIR_PARENT_INVALID');
  ...
  private reconcileFailedReview(planId: string): PlanAutomationResult {
    ...
    if (!candidate?.resultRevision) throw new ForgeFlowError('PLAN_REVIEW_RECOVERY_CANDIDATE_MISSING');
    ...
    if (
      ...
      throw new ForgeFlowError('PLAN_REVIEW_RECOVERY_NOT_REQUIRED');
    ...
    if (!Number.isInteger(attempt) || attempt < 1)
      throw new ForgeFlowError('PLAN_REVIEW_RECOVERY_EVIDENCE_INVALID');
  ...
  createInitialExecution(
    ...
    if (
      ...
      throw new ForgeFlowError('PLAN_NOT_AUTOMATABLE');
    if (workItem.planId !== plan.planId) throw new ForgeFlowError('EXECUTION_WORK_ITEM_MISMATCH');
  ...
  private recoverReviewVerdict(review: Review, reviewer: Execution): Review {
    ...
    if (!evidence) throw new ForgeFlowError('REVIEW_COMPLETION_EVIDENCE_MISSING');
    ...
    if (!result.value) throw new ForgeFlowError('REVIEW_VERDICT_PERSIST_FAILED');
    return result.value;
  }
  ...
  private createReview(
    ...
    if (!candidate.resultRevision) throw new ForgeFlowError('REVIEW_EXACT_RESULT_REQUIRED');
    const reviewId = stableId(
      'review',
      candidate.identity.executionId,
      candidate.resultRevision,
  ...
  private async completePlan(
    ...
    if (current.delivery && !isDeliveryComplete(current.delivery)) {
      ...
      try {
        ...
        if (!durable) throw new ForgeFlowError('DELIVERY_OBSERVATION_PERSIST_FAILED');
        ...
        const code = error instanceof ForgeFlowError ? error.code : 'DELIVERY_AUTOMATION_FAILED';
  ...
  private async supersedeDeliveryFromVerifiedLineage(plan: Plan): Promise<Plan> {
    ...
    if (!this.workspace.isRevisionAncestor)
      throw new ForgeFlowError('WORKSPACE_ANCESTRY_CHECK_UNAVAILABLE');
    ...
    if (descendants.length > 1) throw new ForgeFlowError('DELIVERY_SUPERSEDING_SIBLING_AMBIGUOUS');
  ...
  private supersedeDeliveryFromVerifiedChild(plan: Plan): Plan {
    ...
    if (candidates.length > 1) throw new ForgeFlowError('DELIVERY_SUPERSEDING_CHILD_AMBIGUOUS');
  ...
  private createDeliveryRepair(
    ...
    if (!delivery) throw new ForgeFlowError('PLAN_DELIVERY_REQUIRED');
    ...
    if (!graph) throw new ForgeFlowError('GRAPH_CREATE_FAILED');
  ...
  private requirePolicy(projectKey: string): PlanAutomationPolicy {
    ...
    if (!policy) throw new ForgeFlowError('PLAN_POLICY_UNAVAILABLE');
    return policy;
  }
}
