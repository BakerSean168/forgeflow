...
4. **Focused reconcilers — completed** — a single lifecycle manager schedules independently testable Plan, Resource, Runtime Admission, Supervisor, Improvement, and Storage controllers; bootstrap owns no feature timers.
5. **Integration packages — completed** — Provider, Workspace, Resource, Delivery, Release, and Intake concrete I/O live under `src/integrations/`; Provider uses exact-one registry selection, while other capabilities use narrow package assemblies/ports. Phase 6 retired the former `core/adapters/*` compatibility layer; provider-neutral Improvement/self-change ports now live under `src/core/maintenance/`.
6. **Typed client — completed and production-attested in v1.2.0** — `@forgeflow/client` is generated from the committed OpenAPI contract, carries exact contract provenance, has no server-internal dependency, and is protected by codegen/type/runtime/build/pack CI gates.
7. **V1 contract hardening — completed and production-attested in v1.3.1** — all 45 public operations have stable unique `operationId` values, explicit 2xx JSON response contracts, an audited 18-request-body set with four preserved optional bodies, generated semantic operation types, and compatibility checks against both the v1.2.1 legacy floor and v1.3.1 hardened floor. Convenience methods may be generated later but cannot become a second DTO authority.
8. **Semantic SDK operations — completed and production-attested in v1.4.0** — the client code generator derives stable `client.operations.<operationId>()` methods from the same committed OpenAPI contract while preserving the raw path/method client. Request/response typing remains generated; no second DTO authority is introduced.
