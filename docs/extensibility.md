...
6. **Typed client — implemented** — `@forgeflow/client` is generated from the committed OpenAPI contract, carries exact contract provenance, has no server-internal dependency, and is protected by codegen/type/runtime/build/pack CI gates. Semantic convenience methods remain downstream of future stable `operationId` declarations rather than becoming a second DTO authority.
