...
8. **Semantic SDK operations — implemented for v1.4.0** — the client code generator derives stable `client.operations.<operationId>()` methods from the same committed OpenAPI contract while preserving the raw path/method client. Request/response typing remains generated; no second DTO authority is introduced.
