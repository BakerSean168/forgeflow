...
When a failed candidate reveals that the original Plan encoded a narrower write scope than its authoritative implementation specification, the same request may carry optional `scopeAmendments`. This is an operator correction, not a model escape hatch: it is accepted only for FAILED/BLOCKED unaccepted WorkItems with no active Execution, must CAS-match `expectedWriteScopes`, may only add exact path/prefix scopes, and is rejected if the widened scope would conflict with a sibling already assigned to the same parallel wave. Every accepted correction emits `WORK_ITEM_WRITE_SCOPES_AMENDED` with old/new scopes and a bounded reason. The amendment is durable independently of later provider/Git recovery; `retry-finalization` still recomputes the real `sourceRevision..HEAD` file set and fails if the candidate remains outside the amended scope.
