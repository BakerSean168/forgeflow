...
- literal worktree provisioning now treats tracked Git submodules as controller-owned topology: exact HEAD gitlinks are initialized recursively before writer access, nested gitfiles are source-owned/read-only, and retry handoff reconstructs corrupted nested metadata only from committed gitlink authority; canonical local `.git/modules/*` stores are preferred as temporary submodule URL overrides, so private submodule provisioning/recovery does not depend on interactive credentials or network re-cloning when the exact pinned object already exists locally;
- a real Digital Biome attempt exposed a post-commit finalization failure after the model rewrote an uninitialized submodule gitfile to a temporary path; `retry-finalization` now preserves a clean committed same-wave candidate without rewriting the failed Execution, while a sibling whose failed writer has already been safely abandoned may participate only from a `QUIESCENT`, ownerless worktree whose HEAD/current revision exactly equal the original source; unowned candidate drift is rejected;
