...
state_file="${FORGEFLOW_HOST_CACHE_STATE_FILE:-/var/lib/forgeflow/host-cache-maintenance.json}"
control_plane_url="${FORGEFLOW_CONTROL_PLANE_URL:-http://127.0.0.1:8420}"
filesystem_path="${FORGEFLOW_HOST_CACHE_FILESYSTEM:-/}"
trigger_free_bytes="${FORGEFLOW_HOST_CACHE_TRIGGER_FREE_BYTES:-17179869184}"
target_free_bytes="${FORGEFLOW_HOST_CACHE_TARGET_FREE_BYTES:-25769803776}"
build_cache_age="${FORGEFLOW_HOST_CACHE_BUILD_AGE:-24h}"
image_age="${FORGEFLOW_HOST_CACHE_IMAGE_AGE:-168h}"
maintenance_lock="${FORGEFLOW_HOST_CACHE_LOCK:-/tmp/forgeflow-host-cache.lock}"
release_lock="${FORGEFLOW_RELEASE_LOCK:-/tmp/forgeflow-release.lock}"
test_mode="${FORGEFLOW_HOST_CACHE_TEST_MODE:-false}"
...
active_execution_count() {
  local payload
  payload="$(curl -fsS --max-time 5 "$control_plane_url/api/v1/executions?status=RUNNING&limit=1")" || return 1
...
if (( active > 0 )); then
  write_state "SKIPPED_ACTIVE_EXECUTION" "FORGEFLOW_EXECUTION_RUNNING" "$before" "$before" "$active"
  exit 0
fi
