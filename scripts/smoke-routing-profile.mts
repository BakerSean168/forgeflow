...
import { createExecutionResourceSelection } from '../src/core/domain/resourceRouting.ts';

const SOURCE_REPOSITORY =
  process.env.FORGEFLOW_ROUTING_SMOKE_REPOSITORY ?? '/home/dev/projects/forgeflow-smoke';
const PRODUCTION_CONTROL_PLANE =
  process.env.FORGEFLOW_ROUTING_SMOKE_CONTROL_PLANE ?? 'http://127.0.0.1:8320';
const MANAGED_WORKSPACE_ROOT = '/var/lib/forgeflow/workspaces';
...
async function requireProductionAdmission(target: TargetProfile): Promise<void> {
  const response = await fetch(`${PRODUCTION_CONTROL_PLANE}/api/v1/runtime-admission`);
...
function isolatedEnv(): NodeJS.ProcessEnv {
  for (const required of ['SESSION_API_KEY', 'FORGEFLOW_LITELLM_API_KEY'])
  ...
  return {
    ...
    FORGEFLOW_SUPERVISOR_RUNTIME_ENABLED: 'false',
    FORGEFLOW_RESOURCE_SELECTOR_ENABLED: 'true',
    FORGEFLOW_RUNTIME_ADMISSION_ENABLED: 'false',
    FORGEFLOW_SINGLE_ACTIVE_PLAN_ENABLED: 'false',
    FORGEFLOW_LITERAL_WORKTREES_ENABLED: 'false',
    FORGEFLOW_BUSINESS_RESOURCE_ENABLED: 'false',
    FORGEFLOW_ANTIGRAVITY_RESOURCE_ENABLED: 'false',
    FORGEFLOW_ALLOWED_REPOSITORY_ROOTS: '/home/dev/projects',
    FORGEFLOW_WORKSPACE_HOST_ROOT: MANAGED_WORKSPACE_ROOT,
    FORGEFLOW_WORKSPACE_EXECUTION_ROOT: '/workspace',
    FORGEFLOW_WORKSPACE_UID: '10001',
    FORGEFLOW_WORKSPACE_GID: '10001',
    FORGEFLOW_AUTOMATION_PROJECTS: 'forgeflow-smoke',
    FORGEFLOW_REQUIRE_DELIVERY: 'false',
    FORGEFLOW_OPENHANDS_URL: OPENHANDS_URL,
...
async function runImplementation(runtime: any, baseRevision: string, target: TargetProfile) {
  ...
  const objective = [
    ...
    'Leave the repository clean and write the required ForgeFlow implementation completion evidence.',
  ].join(' ');
...
if (!target)
  fail(
    `usage: npx tsx model-control-plane/scripts/smoke-routing-profile.mts <${Object.keys(TARGETS).join('|')}>`,
  );
...
try {
  ...
  for (const session of runtime.repositories.sessions.listByPlan(
    ...
    if (hostPath?.startsWith(`${MANAGED_WORKSPACE_ROOT}/forgeflow/executions/`))
