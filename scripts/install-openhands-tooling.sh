...
docker exec "$CONTAINER" sh -lc \
  "mkdir -p '$TOOL_ROOT' && npm install --prefix '$TOOL_ROOT' --no-audit --no-fund \
    ...
    >/tmp/forgeflow-tooling-install.log 2>&1"
...
# policy for ForgeFlow, so omission must remain omission on the wire. Apply a
# version-pinned, drift-detecting vendor patch after every install/check.
docker exec "$CONTAINER" node \
  /opt/forgeflow-tools/patch_dsh_no_default_max_tokens.mjs \
  "$DSH_ROOT/node_modules/@deepseek-ai/dsh-llm-deepseek"
docker exec "$CONTAINER" node \
  /opt/forgeflow-tools/verify_dsh_no_default_max_tokens.mjs \
  "$DSH_ROOT/node_modules/@deepseek-ai/dsh-llm-deepseek"
printf '%-20s' "dsh"
...
# ZCode ACP receives execution-scoped provider/MCP state from the ForgeFlow launcher;
